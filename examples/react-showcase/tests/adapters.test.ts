...
import type { Logger, Config, MessageStore, UserSession } from "../src/types.js";
...
describe("MessageStoreAdapter", () => {
  ...
  it("addMessage() notifies subscribers with updated messages", async () => {
    ...
    if (!firstCall) throw new Error("Expected subscriber to be called");
...
describe("ChatServiceAdapter", () => {
  ...
  it("sends message with correct sender info from UserSession", async () => {
    ...
    const addMessageMock = mockMessageStore.addMessage as ReturnType<typeof vi.fn>;
    const firstCall = addMessageMock.mock.calls[0];
    if (!firstCall) throw new Error("Expected addMessage to be called");
