...
export function rewriteExternalImports(code: string): string {
  ...
  let result = code.replace(
    ...
    (_match, names: string, pkg: string) => {
      ...
      // Convert import alias syntax (X as Y) to destructuring syntax (X: Y)
      const destructured = cleaned.replace(/(\w+)\s+as\s+(\w+)/g, "$1: $2");
      return `const { ${destructured} } = globalThis.__hexModules["${pkg}"];`;
    }
  );
