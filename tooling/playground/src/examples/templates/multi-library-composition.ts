...
import type { UserService } from "./ports/user-service";
import { createConsoleLogger } from "./adapters/console-logger";
import { createMemoryCache } from "./adapters/memory-cache";
import { createUserService } from "./adapters/user-service-impl";

// --- Tagged error types for each fallible factory ---

interface LoggerInitError {
  readonly _tag: "LoggerInitError";
  readonly reason: string;
}

interface CacheInitError {
  readonly _tag: "CacheInitError";
  readonly reason: string;
}

interface UserServiceInitError {
  readonly _tag: "UserServiceInitError";
  readonly reason: string;
}
...
const loggerAdapter = createAdapter({
  provides: LoggerPort,
  factory: (): FactoryResult<Logger, LoggerInitError> => {
    return { _tag: "Ok", value: createConsoleLogger() };
  },
  lifetime: "singleton",
});
...
const cacheAdapter = createAdapter({
  provides: CachePort,
  requires: [LoggerPort],
  factory: ({ Logger }): FactoryResult<Cache, CacheInitError> => {
    return { _tag: "Ok", value: createMemoryCache(Logger) };
  },
  lifetime: "singleton",
});
...
const userServiceAdapter = createAdapter({
  ...
  factory: ({ Logger, Cache }): FactoryResult<UserService, UserServiceInitError> => {
    return { _tag: "Ok", value: createUserService(Logger, Cache) };
  },
  lifetime: "singleton",
});
...
const loggerFallback = createAdapter({
  provides: LoggerPort,
  factory: () => createConsoleLogger(),
  lifetime: "singleton",
});

const cacheFallback = createAdapter({
  provides: CachePort,
  requires: [LoggerPort],
...
const userServiceFallback = createAdapter({
  provides: UserServicePort,
  requires: [LoggerPort, CachePort],
...
// Compose everything into a single graph using adapterOrElse
const graph = GraphBuilder.create()
  .provide(adapterOrElse(loggerAdapter, loggerFallback))
  .provide(adapterOrElse(cacheAdapter, cacheFallback))
  .provide(adapterOrElse(userServiceAdapter, userServiceFallback))
  .build();
