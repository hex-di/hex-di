 ...
 * import { toDotGraph } from "@hex-di/visualization";
 *
...
export function toDotGraph(inspection: GraphInspection, options: DotGraphOptions = {}): string {
  ...
  for (const [portName, deps] of Object.entries(inspection.dependencyMap) as [
    string,
    readonly string[],
  ][]) {
 ...
 * ```typescript
 * import { GraphBuilder, inspectGraph } from "@hex-di/graph";
 * import { toMermaidGraph } from "@hex-di/visualization";
 *
 * const graph = GraphBuilder.create()
 *   .provide(LoggerAdapter)
 *   .provide(DatabaseAdapter)
