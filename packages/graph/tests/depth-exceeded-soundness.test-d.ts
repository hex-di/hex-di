 ...
 * 2. `withExtendedDepth()` returns WARNING instead of ERROR
 *
 * @see ARCHITECTURE.md for discussion of the soundness hole fix
 */
import { describe, it, expectTypeOf } from "vitest";
import { GraphBuilder } from "../src/index.js";
import type { DepthLimitError, DepthLimitWarning, BuilderInternals } from "./test-types.js";
...
describe("Depth-exceeded returns WARNING with unsafe override", () => {
  ...
  it("withExtendedDepth creates builder with unsafe flag", () => {
  ...
  it("withMaxDepth().withExtendedDepth() chains correctly", () => {
  ...
  it("forParent preserves unsafe depth override through factory", () => {
    ...
    const builder = GraphBuilder.withExtendedDepth().forParent(parentGraph);
...
describe("merge() respects UnsafeDepthOverride flag", () => {
  ...
  it("merge() with unsafe override builder returns WARNING on depth exceeded", () => {
    ...
    const builder = GraphBuilder.withMaxDepth<2>().withExtendedDepth().create();
