 ...
 * @internal
 */
type IsGreaterThan<TLevelA extends number, TLevelB extends number> = TLevelA extends SINGLETON_LEVEL
  ? false // SINGLETON_LEVEL is never greater than anything (Singleton = longest lived)
  : TLevelA extends SCOPED_LEVEL
    ? TLevelB extends SINGLETON_LEVEL
      ? true // SCOPED_LEVEL > SINGLETON_LEVEL: Scoped > Singleton
      : false // SCOPED_LEVEL <= SCOPED_LEVEL, SCOPED_LEVEL <= TRANSIENT_LEVEL
    : TLevelA extends TRANSIENT_LEVEL
      ? TLevelB extends SINGLETON_LEVEL | SCOPED_LEVEL
        ? true // TRANSIENT_LEVEL > SINGLETON_LEVEL, TRANSIENT_LEVEL > SCOPED_LEVEL
        : false // TRANSIENT_LEVEL <= TRANSIENT_LEVEL
      : false;
