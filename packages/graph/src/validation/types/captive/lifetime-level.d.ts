...
export type DiagnosticLifetimeLevel<TLifetime> = [
    TLifetime
] extends [never] ? never : TLifetime extends "singleton" ? SINGLETON_LEVEL : TLifetime extends "scoped" ? SCOPED_LEVEL : TLifetime extends "transient" ? TRANSIENT_LEVEL : InferenceError<"LifetimeLevel", "Invalid lifetime. Expected 'singleton', 'scoped', or 'transient'.", TLifetime>;
/**
...
export type DiagnosticLifetimeName<Level> = [
    Level
] extends [never] ? never : Level extends SINGLETON_LEVEL ? "Singleton" : Level extends SCOPED_LEVEL ? "Scoped" : Level extends TRANSIENT_LEVEL ? "Transient" : InferenceError<"LifetimeName", "Invalid level. Expected 1, 2, or 3.", Level>;
