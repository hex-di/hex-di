...
export type IsBrandedPortName<T> = [
    T
] extends [never] ? false : T extends {
    readonly [__portNameBrand]: infer _TBrand;
} ? true : false;
/**
...
} ? TName extends string ? BrandedPortName<TName> : never : never;
/**
...
export type DiagnosticAdapterProvidesName<TAdapter> = [
    TAdapter
] extends [never] ? never : TAdapter extends {
    provides: {
        __portName: infer TName;
    };
} ? TName extends string ? TName : InferenceError<"AdapterProvidesName", "Adapter provides.__portName must be a string literal type.", TAdapter> : InferenceError<"AdapterProvidesName", "Adapter must have a 'provides' property with a Port type containing __portName.", TAdapter>;
/**
...
export type BrandedAdapterRequiresNames<TAdapter> = TAdapter extends {
    ...
    __portName: infer TName;
} ? TName extends string ? BrandedPortName<TName> : never : never : MalformedAdapterError<"missing-requires">;
/**
