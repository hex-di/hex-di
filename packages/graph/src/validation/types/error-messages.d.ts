...
import type { CaptiveDependencyError, ReverseCaptiveDependencyError, MalformedAdapterError, ForwardReferenceMarker } from "./captive/errors.js";
/**
 ...
 * @internal
 */
export type ExtractPortNames<TPorts> = [TPorts] extends [never] ? never : TPorts extends Port<unknown, infer Name> ? Name : never;
/**
 ...
 * @internal
 */
type LastOfUnion<U> = UnionToIntersectionFn<U extends unknown ? (x: U) => void : never> extends (x: infer L) => void ? L : never;
/**
 ...
 * @internal
 */
type JoinPortNamesDepth<D extends unknown[] = []> = D["length"] extends JoinPortNamesMaxDepth ? true : false;
/**
 ...
 * @internal
 */
export type JoinPortNames<T, Acc extends string = "", Depth extends unknown[] = []> = JoinPortNamesDepth<Depth> extends true ? `${Acc}...(truncated: depth limit reached)` : [T] extends [never] ? Acc : LastOfUnion<T> extends Port<unknown, infer N extends string> ? JoinPortNames<Exclude<T, LastOfUnion<T>>, [Acc] extends [""] ? N : `${Acc}, ${N}`, [...Depth, unknown]> : Acc;
/**
 ...
 * @internal
 */
export type MissingDependencyError<MissingPorts> = [MissingPorts] extends [never] ? never : {
 ...
 * @internal
 */
export type DepthLimitExceededMarker<TMaxDepth extends number = 50, TLastPort extends string = "unknown"> = {
