...
> =
  ...
  GetDirectDeps<TDepGraph, TFrom> extends infer Deps
    ? // Step 2: Only continue if dependencies exist and are strings
      Deps extends string
      ? // Step 3: Delegate to FindCyclePath with updated state
        FindCyclePath<
          TDepGraph,
          Deps, // New starting points (distributes if union)
          TTarget,
          CyclePathNextSegment<TPath, TFrom>, // Append current port to path
          TVisited | TFrom, // Mark current port as visited
          IncrementDepth<TDepth>,
          TMaxDepth
        >
      : never // No valid dependencies, dead end
    : never;
...
export type FindCyclePath<
  ...
  TMaxDepth extends number = DefaultMaxDepth,
> =
  // Base Case 1: Check depth limit - return truncated path with warning
  DepthExceeded<TDepth, TMaxDepth> extends true
    ? CyclePathDepthExceeded<TPath, TFrom, TMaxDepth>
    : // Base Case 2: Skip if already visited (prune this branch)
      TFrom extends TVisited
      ? never
      : // Base Case 3: Found the target - return complete cycle path
        TFrom extends TTarget
        ? CyclePathFound<TPath, TTarget>
        : // Recursive Case: Explore dependencies
          TFrom extends string
          ? CyclePathRecurse<TDepGraph, TFrom, TTarget, TPath, TVisited, TDepth, TMaxDepth>
          : never;
...
export type BuildCyclePath<
  ...
  TMaxDepth extends number = DefaultMaxDepth,
> = FindCyclePath<TDepGraph, TRequires, TProvides, TProvides, never, [], TMaxDepth>;
 ...
 * @internal
 */
type ExtractFirstEdge<TPath extends string> = TPath extends `${infer From} -> ${infer Rest}`
  ? Rest extends `${infer To} -> ${infer _Remaining}`
    ? { from: From; to: To; remaining: Rest }
    : Rest extends `${infer To}`
      ? { from: From; to: To; remaining: never }
      : never
  : never;
 ...
 * @internal
 */
export type FormatLazySuggestion<
  TFrom extends string,
  TTo extends string,
> = `lazyPort(${TTo}) in ${TFrom}Adapter`;
 ...
 * @internal
 */
type CollectSuggestions<TPath extends string, TAcc extends string = ""> =
  // Step 1: Try to extract the first edge from the path
  ExtractFirstEdge<TPath> extends {
    from: infer From extends string;
    to: infer To extends string;
    remaining: infer Rest;
  }
    ? // Step 2: Check if there are more edges to process
      Rest extends string
      ? // Recursive Case: More edges remain, accumulate and continue
        CollectSuggestions<
          Rest,
          TAcc extends ""
            ? FormatLazySuggestion<From, To>
            : `${TAcc}, or ${FormatLazySuggestion<From, To>}`
        >
      : // Base Case: Last edge, append and return
        TAcc extends ""
        ? FormatLazySuggestion<From, To>
        : `${TAcc}, or ${FormatLazySuggestion<From, To>}`
    : // Base Case: No more edges to extract, return accumulated
      TAcc;
 ...
 * ```
 */
export type FormatLazySuggestionMessage<TPath extends string> =
  LazySuggestions<TPath> extends infer S extends string ? (S extends "" ? "" : `Use ${S}.`) : "";
