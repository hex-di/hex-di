 ...
 * @internal
 */
export type MergeConflictError<
 ...
 * ```
 */
export type MergeConflictErrorMessage<
  TPort extends string,
  TDeps1 extends string,
  TDeps2 extends string,
> = `ERROR[HEX022]: Merge conflict for port '${TPort}': Graph A declares dependencies [${FormatDeps<TDeps1>}], Graph B declares dependencies [${FormatDeps<TDeps2>}]. These cannot be safely merged. Fix: Ensure both graphs use the same adapter for '${TPort}', or remove one before merging.`;
 ...
 * @internal
 */
export type DetectMergeConflict<G1, G2> =
  // Step 1: Get common keys
  CommonKeys<G1, G2> extends infer Keys extends string
    ? // Step 2: If no common keys, no conflict possible
      IsNever<Keys> extends true
      ? never
      : // Step 3: Check each common key for conflicts
        FindFirstConflict<G1, G2, Keys>
    : never;
 ...
 * @internal
 */
type FindFirstConflict<G1, G2, Keys extends string> = Keys extends string
  ? // For each key, check if deps are equal
    CheckPortConflict<G1, G2, Keys>
  : never;
 ...
 * @internal
 */
type CheckPortConflict<G1, G2, K extends string> = K extends keyof G1 & keyof G2
  ? // Get the dependencies from both graphs (non-distributively via infer)
    G1[K] extends infer Deps1
    ? G2[K] extends infer Deps2
      ? // Check if both are never (no deps)
        IsNever<Deps1> extends true
        ? IsNever<Deps2> extends true
          ? never // Both have no deps = compatible
          : MergeConflictError<K, ExtractString<Deps1>, ExtractString<Deps2>> // G1 has none, G2 has some
        : // Deps1 is not never
          IsNever<Deps2> extends true
          ? MergeConflictError<K, ExtractString<Deps1>, ExtractString<Deps2>> // G1 has some, G2 has none
          : // Both have deps - check if equal (using tuple wrap to prevent distribution)
            StringUnionEqual<Deps1 & string, Deps2 & string> extends true
            ? never // Same deps = no conflict
            : MergeConflictError<K, ExtractString<Deps1>, ExtractString<Deps2>> // Different deps = conflict!
      : never
    : never
  : never;
