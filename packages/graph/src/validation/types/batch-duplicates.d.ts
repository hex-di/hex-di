 ...
 * @internal
 */
export type HasDuplicatesInBatch<TAdapters extends readonly unknown[]> = TAdapters extends readonly [infer Head, ...infer Tail] ? Tail extends readonly [] ? false : HasOverlapWithRest<InferAdapterProvides<Head>, Tail> extends true ? true : HasDuplicatesInBatch<Tail> : false;
/**
 ...
 * @internal
 */
type HasOverlapWithRest<TPort, TRest extends readonly unknown[]> = [TPort] extends [never] ? false : TPort extends InferManyProvides<TRest> ? true : false;
/**
...
export type FindBatchDuplicate<TAdapters extends readonly unknown[]> = TAdapters extends readonly [
    infer Head,
    ...infer Tail
] ? Tail extends readonly [] ? never : FindOverlapWithRest<InferAdapterProvides<Head>, Tail> extends never ? FindBatchDuplicate<Tail> : FindOverlapWithRest<InferAdapterProvides<Head>, Tail> : never;
/**
 * Finds a port that overlaps with ports provided by remaining adapters.
 *
 * @internal
 */
type FindOverlapWithRest<TPort, TRest extends readonly unknown[]> = [TPort] extends [never] ? never : TPort extends InferManyProvides<TRest> ? TPort : never;
/**
