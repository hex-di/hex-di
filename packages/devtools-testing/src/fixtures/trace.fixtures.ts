 ...
 *
 * These factories help create test data for trace entries and statistics
 ...
 * ```typescript
 * const trace = createTraceEntry({ id: "trace-1" });
 * // {
 * //   id: "trace-1",
 * //   portName: "trace-1",
 * //   lifetime: "singleton",
 ...
 * ```
 *
 * @example Customized trace entry
 * ```typescript
 ...
 * expect(traces[2].childIds).toEqual([traces[0].id, traces[1].id]);
 * ```
 *
 ...
 * @example
 * ```typescript
 * const traces = createRepeatedTraces("UserService", 5, { lifetime: "transient" });
 ...
 * @example
 * ```typescript
 * const hierarchy = createTraceHierarchy(
 *   { id: "trace-parent", portName: "Parent" },
 *   [
 *     { id: "trace-child-1", portName: "Child1" },
 *     { id: "trace-child-2", portName: "Child2" },
 *   ]
 * );
 *
 * expect(hierarchy.parent.childIds).toEqual(["trace-child-1", "trace-child-2"]);
 * expect(hierarchy.children[0].parentId).toBe("trace-parent");
 * ```
 *
