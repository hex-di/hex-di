...
/**
 * Async factory resolution status.
 */
export type AsyncFactoryStatus = "pending" | "resolved" | "error" | null;
...
export interface ServiceInfoViewModel {
  ...
  /** Async factory resolution status (null for sync factories) */
  readonly asyncFactoryStatus?: AsyncFactoryStatus;
  /** Async factory resolution time in ms (null if not resolved or sync) */
  readonly asyncResolutionTime?: number | null;
  /** Chain of services affected by captive dependency, if any */
  readonly captiveChain?: readonly string[];
}
...
// =============================================================================
// Extended Service Info View Model
// =============================================================================

/**
 * Extended service information with async factory and captive dependency details.
 *
 * Adds async factory resolution status and captive dependency chain
 * visualization to the base ServiceInfoViewModel.
 */
export interface ExtendedServiceInfoViewModel extends ServiceInfoViewModel {
  /** Async factory resolution status (null for sync factories) */
  readonly asyncFactoryStatus: AsyncFactoryStatus | null;
  /** Async factory resolution time in ms (null if not resolved or sync) */
  readonly asyncResolutionTime: number | null;
  /** Chain of services affected by captive dependency, if any */
  readonly captiveChain: readonly string[];
...
/**
 * Creates an empty ExtendedServiceInfoViewModel.
 */
export function createEmptyExtendedServiceInfoViewModel(): ExtendedServiceInfoViewModel {
  return Object.freeze({
    portName: "",
    lifetime: "singleton",
    factoryKind: "sync",
