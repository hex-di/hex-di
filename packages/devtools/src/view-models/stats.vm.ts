...
/**
 * Memory tracking information for instance counts.
 */
export interface MemoryTrackingViewModel {
  /** Estimated singleton instance count */
  readonly singletonCount: number;
  /** Estimated scoped instance count across all active scopes */
  readonly scopedCount: number;
  /** Number of active scopes */
  readonly activeScopeCount: number;
  /** Singleton pool size (unique services) */
  readonly singletonPoolSize: number;
  /** Distribution of scoped instances per scope */
  readonly scopedDistribution: {
    readonly min: number;
    readonly max: number;
    readonly avg: number;
  };
  /** Memory growth trend over time */
  readonly growthTrend: "increasing" | "stable" | "decreasing" | "unknown";
}
...
export interface StatsViewModel {
  ...
  readonly memoryTracking: MemoryTrackingViewModel;
...
/**
 * Creates an empty MemoryTrackingViewModel.
 */
function createEmptyMemoryTracking(): MemoryTrackingViewModel {
  return Object.freeze({
    singletonCount: 0,
    scopedCount: 0,
    activeScopeCount: 0,
...
export function createEmptyStatsViewModel(): StatsViewModel {
  return Object.freeze({
    ...
    memoryTracking: createEmptyMemoryTracking(),
    sessionStart: "-",
    sessionDuration: "-",
    isEmpty: true,
    lastUpdated: "-",
