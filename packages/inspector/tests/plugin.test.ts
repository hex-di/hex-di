...
import { createInspectorPlugin, INSPECTOR, hasInspector, getInspectorAPI } from "../src/index.js";
...
describe("InspectorPlugin registration", () => {
  it("registers and provides INSPECTOR symbol access after binding", () => {
    ...
    // INSPECTOR symbol should be accessible
  ...
  it("throws if API methods are called before binding", () => {
    const graph = createTestGraph();
    const { plugin } = createInspectorPlugin();
    const container = createContainer(graph, {
      plugins: [plugin],
    });

    // Should throw because container is not bound
    expect(() => container[INSPECTOR].getSnapshot()).toThrow(
      "InspectorPlugin not bound to container"
    );
  });

  it("throws if bindContainer is called twice", () => {
    const graph = createTestGraph();
    const { plugin, bindContainer } = createInspectorPlugin();
    const container = createContainer(graph, {
      plugins: [plugin],
    ...
    expect(() => bindContainer(container)).toThrow("InspectorPlugin already bound to a container");
  });

  it("creates independent plugin instances", () => {
    const graph = createTestGraph();

    const inspector1 = createInspectorPlugin();
    const inspector2 = createInspectorPlugin();

    const container1 = createContainer(graph, { plugins: [inspector1.plugin] });
    const container2 = createContainer(graph, { plugins: [inspector2.plugin] });

    inspector1.bindContainer(container1);
    inspector2.bindContainer(container2);

    container1.resolve(LoggerPort);

    // Each container should have independent inspection state
...
describe("scope events", () => {
  it("emits scope-created events when scopes are created", () => {
    const graph = createTestGraph();
    const { plugin, bindContainer } = createInspectorPlugin();
    const container = createContainer(graph, {
      plugins: [plugin],
