...
describe("ResultAsync", () => {
  ...
  it("ResultAsync.err('x') resolves to Err('x')", async () => {
    const result = await ResultAsyncImpl.err("x");
  ...
  it("resultAsync.map(v => v * 2) transforms Ok value", async () => {
    const result = await ResultAsyncImpl.ok(21).map(v => v * 2);
    expect(result._tag).toBe("Ok");
    if (result.isOk()) expect(result.value).toBe(42);
  });
  ...
  it("resultAsync.map(async v => v * 2) accepts async transform", async () => {
    const result = await ResultAsyncImpl.ok(21).map(async v => v * 2);
    expect(result._tag).toBe("Ok");
    if (result.isOk()) expect(result.value).toBe(42);
  });
  ...
  it("resultAsync.mapErr(e => e.toUpperCase()) transforms Err value", async () => {
    const result = await ResultAsyncImpl.err("fail").mapErr(e => e.toUpperCase());
    expect(result._tag).toBe("Err");
  ...
  it("resultAsync.andThen(v => ok(v + 1)) chains sync Result", async () => {
    const result = await ResultAsyncImpl.ok(41).andThen(v => ok(v + 1));
    expect(result._tag).toBe("Ok");
    if (result.isOk()) expect(result.value).toBe(42);
  });
  ...
  it("resultAsync.andThen(v => ResultAsync.ok(v + 1)) chains async Result", async () => {
    const result = await ResultAsyncImpl.ok(41).andThen(v => ResultAsyncImpl.ok(v + 1));
    expect(result._tag).toBe("Ok");
    if (result.isOk()) expect(result.value).toBe(42);
  });
  ...
  it("resultAsync.orElse(e => ok(99)) recovers from error", async () => {
    const result = await ResultAsyncImpl.err("x").orElse(() => ok(99));
    expect(result._tag).toBe("Ok");
  ...
  it("resultAsync.andTee(v => {}) calls side effect, returns original", async () => {
    const fn = vi.fn();
    const result = await ResultAsyncImpl.ok(42).andTee(fn);
  ...
  it("resultAsync.orTee(e => {}) calls side effect on Err", async () => {
    const fn = vi.fn();
    const result = await ResultAsyncImpl.err("x").orTee(fn);
  ...
  it("resultAsync.andThrough(v => ok('x')) returns original on Ok", async () => {
    const result = await ResultAsyncImpl.ok(42).andThrough(() => ok("x"));
    expect(result._tag).toBe("Ok");
    if (result.isOk()) expect(result.value).toBe(42);
  });
  ...
  it("resultAsync.andThrough(v => err('fail')) propagates Err", async () => {
    const result = await ResultAsyncImpl.ok(42).andThrough(() => err("fail"));
    expect(result._tag).toBe("Err");
    if (result.isErr()) expect(result.error).toBe("fail");
  });
  ...
  it("resultAsync.match(onOk, onErr) returns Promise", async () => {
    const value = await ResultAsyncImpl.ok(42).match(
      v => v + 1,
      () => 0
    );
  ...
  it("resultAsync.unwrapOr(0) returns Promise of value or default", async () => {
    expect(await ResultAsyncImpl.ok(42).unwrapOr(0)).toBe(42);
    expect(await ResultAsyncImpl.err("x").unwrapOr(0)).toBe(0);
  });

  // DoD 8 #20
  it("resultAsync.toNullable() returns Promise of value or null", async () => {
    expect(await ResultAsyncImpl.ok(42).toNullable()).toBe(42);
    expect(await ResultAsyncImpl.err("x").toNullable()).toBeNull();
  });

  // DoD 8 #21
  it("resultAsync.intoTuple() returns Promise of tuple", async () => {
    expect(await ResultAsyncImpl.ok(42).intoTuple()).toEqual([null, 42]);
    expect(await ResultAsyncImpl.err("x").intoTuple()).toEqual(["x", null]);
  });
  ...
  it("Chaining multiple async operations preserves error accumulation", async () => {
    type _E1 = { _tag: "E1" };
    type E2 = { _tag: "E2" };
    const result = await ResultAsyncImpl.ok(1)
      .andThen(v => ok(v + 1))
  ...
  it("ok(42).asyncAndThen(v => ResultAsync.ok(v * 2)) returns ResultAsync", async () => {
    const result = await ok(42).asyncAndThen(v => ResultAsyncImpl.ok(v * 2));
    expect(result._tag).toBe("Ok");
    if (result.isOk()) expect(result.value).toBe(84);
  });
});
