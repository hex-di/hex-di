...
export function isRuntimeContainer(
  ...
  return "initialize" in resolver && typeof (resolver as RuntimeContainer).initialize === "function";
}
  ...
  const wrapped: RuntimeContainer = {
    ...
    initialize: async () => {
      if (container.initialize) {
        const initialized = await container.initialize();
        return toRuntimeResolver(initialized as ResolverLike);
      }
      return base;
    },
    get isInitialized() {
...
function createTypedResolverWrapper<TProvides extends Port<unknown, string>>(
  ...
  return {
    ...
    resolveAsync<P extends TProvides>(port: P): Promise<InferService<P>> {
      return resolver.resolveAsync(port) as Promise<InferService<P>>;
    },
    createScope(): TypedResolver<TProvides> {
      return createTypedResolverWrapper<TProvides>(resolver.createScope());
    },
    dispose(): Promise<void> {
