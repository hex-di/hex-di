/**
 * Type tests for @hex-di/runtime inspector plugin.
 *
 * These tests verify:
...
import {
  ...
  type InspectorContainerSnapshot as ContainerSnapshot,
  type InspectorContainerKind as ContainerKind,
  type InspectorContainerPhase as TypedPhase,
  type InspectorScopeTree as ScopeTree,
} from "../../../src/index.js";
...
describe("ContainerSnapshot discriminated union", () => {
  it("narrows to RootContainerSnapshot when kind is 'root'", () => {
    const container = pipe(createContainer(graph, { name: "Test" }), withInspector);
  ...
  it("narrows to ChildContainerSnapshot when kind is 'child'", () => {
    ...
    if (snapshot.kind === "child") {
      ...
      // inheritanceModes is a Map from port name to inheritance mode
      expectTypeOf(snapshot.inheritanceModes).toMatchTypeOf<
        ReadonlyMap<string, "shared" | "forked" | "isolated">
      >();
    }
  });
...
describe("hasSubscription type guard", () => {
  ...
  it("distinguishes plugin vs standalone inspector", () => {
    ...
    const containerWithPlugin = pipe(createContainer(graph, { name: "Test" }), withInspector);
    ...
    const containerWithout = createContainer(graph, { name: "Test" });
    const standaloneInspector = createInspectorAPI(containerWithout);
    // The return type from createInspectorAPI has subscribe as undefined, not the function
...
// =============================================================================
// Standalone createInspectorAPI Type Tests
// =============================================================================

describe("standalone createInspectorAPI", () => {
  it("returns InspectorAPI", () => {
    const container = createContainer(graph, { name: "Test" });
    const inspector = createInspectorAPI(container);
