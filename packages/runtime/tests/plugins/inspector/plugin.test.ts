...
describe("InspectorPlugin registration", () => {
  ...
  it("provides isolated state per container via singleton plugin", () => {
    ...
    const container1 = pipe(createContainer(graph, { name: "Test" }), withInspector);
    const container2 = pipe(createContainer(graph, { name: "Test" }), withInspector);
...
describe("standalone createInspectorAPI", () => {
  ...
  it("caches inspector instances (returns same instance)", () => {
    ...
    const inspector1 = createInspectorAPI(container);
    const inspector2 = createInspectorAPI(container);
...
describe("type guards", () => {
  ...
  it("hasSubscription distinguishes plugin vs standalone inspector", () => {
    ...
    const standaloneContainer = createContainer(graph, { name: "Test" });
    const pullOnlyInspector = createInspectorAPI(standaloneContainer);
    expect(hasSubscription(pullOnlyInspector)).toBe(false);

    // Plugin - push + pull
    const pluginContainer = pipe(createContainer(graph, { name: "Test" }), withInspector);
