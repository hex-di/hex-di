...
export function getInternalAccessor(container) {
 ...
 * @internal
 */
function buildScopeTreeNode(scopeState, totalCount) {
    const children = [];
...
export function createInspector(container) {
    ...
    function snapshot() {
        ...
        const singletons = [];
        const resolvedPortNames = new Set();
     ...
     */
    function buildContainerScopeTree(state) {
    ...
    function isResolved(portName) {
        ...
        if (adapterInfo === undefined) {
            throw new Error(`Port '${portName}' is not registered in this container. ` +
