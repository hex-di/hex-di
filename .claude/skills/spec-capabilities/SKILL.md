...
Rules and conventions for authoring **capability specification files** in a spec's `capabilities/` directory. Capability files describe _what the user can do_ — they bridge features (the _why_) and behaviors (the _how_) by documenting concrete interaction flows from the user's perspective.
...
6. **No duplicate contracts** — The capability describes the _interaction flow_; the behavior files contain formal MUST/SHALL contracts. Don't duplicate.
