...
describe("useInfiniteQuery initial state fields (mutation killers)", () => {
  it("initial state has all fields correctly set", () => {
    const container = createTestContainer();
    container.register(PaginatedPort, () =>
      ResultAsync.fromSafePromise(new Promise<PageResult>(() => {}))
    );
    ...
    expect(capturedState.hasNextPage).toBe(false);
    expect(capturedState.hasPreviousPage).toBe(false);
    // During initial fetch, isFetchingNextPage may be true (initial = next direction)
    expect(typeof capturedState.isFetchingNextPage).toBe("boolean");
    expect(capturedState.isFetchingPreviousPage).toBe(false);

    client.dispose();
  });

  it("success state has data defined and correct status", async () => {
    const client = createPaginatedClient();
    ...
    expect(capturedState.isFetchingNextPage).toBe(false);
    expect(capturedState.isFetchingPreviousPage).toBe(false);

    client.dispose();
  });

  it("error state has error set and data undefined", async () => {
    const container = createTestContainer();
    container.register(PaginatedPort, () =>
      ResultAsync.err({ _tag: "NetworkError", message: "timeout" })
    );
    const client = createQueryClient({ container, defaults: { retry: 0 } });
  ...
  it("fetchPage passes correct direction for next", async () => {
    const client = createPaginatedClient();
    ...
    function TestComponent() {
      ...
      return (
        <div>
          <span>Pages: {state.data.pages.length}</span>
          <span>PageParams: {state.data.pageParams.length}</span>
          <button onClick={() => void state.fetchNextPage()}>Next</button>
        </div>
      );
    }
  ...
  it("fetchPage for previous prepends to pageParams", async () => {
    const client = createPaginatedClient();
    ...
    function TestComponent() {
      ...
      return (
        <div>
          <span>Pages: {state.data.pages.length}</span>
          <span>FirstParam: {String(state.data.pageParams[0])}</span>
          <button onClick={() => void state.fetchPreviousPage()}>Prev</button>
        </div>
      );
    }
    ...
    await act(async () => fireEvent.click(screen.getByText("Prev")));
    await waitFor(() => expect(screen.getByText("Pages: 2")).toBeDefined());
    // First param should now be "page-0" (prepended)
    expect(screen.getByText("FirstParam: page-0")).toBeDefined();
