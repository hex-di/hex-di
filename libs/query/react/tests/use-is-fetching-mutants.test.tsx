...
describe("useIsFetching (mutation killers)", () => {
  ...
  it("returns exact 1 when single query is fetching", async () => {
    const container = createTestContainer();
    container.register(
      UsersPort,
      () => ResultAsync.fromSafePromise(new Promise<User[]>(() => {})) // never resolves
    ...
    await waitFor(() => expect(capturedCount).toBe(1), { timeout: 3000 });

    client.dispose();
  });

  it("returns 0 when no queries active and not disposed", () => {
    const container = createTestContainer();
    const client = createQueryClient({ container, defaults: { retry: 0 } });
  ...
  it("subscribes to cache updates for re-renders", async () => {
    const container = createTestContainer();
    container.register(UsersPort, () =>
      ResultAsync.fromSafePromise(
        new Promise<User[]>(resolve => setTimeout(() => resolve([{ id: "1", name: "A" }]), 30))
      )
    );
    const client = createQueryClient({ container, defaults: { retry: 0 } });
    ...
    function TestComponent() {
      useQuery(UsersPort, {});
      const c = useIsFetching();
      if (counts.length === 0 || counts[counts.length - 1] !== c) {
        counts.push(c);
      }
      return <div>Count: {c}</div>;
    }
    ...
    await waitFor(() => expect(counts[counts.length - 1]).toBe(0), { timeout: 3000 });

    // Should have seen a transition: at some point count was non-zero, then back to 0
