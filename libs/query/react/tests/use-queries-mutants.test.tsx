...
describe("useQueries (mutation killers)", () => {
  ...
  it("snapshot returns same reference when observers unchanged", async () => {
    const client = createTestClient();
    const snapshots: ReadonlyArray<unknown>[] = [];
    ...
    // Find consecutive snapshots where both are success (same state)
    const successSnaps = snapshots.filter((s: any) => s[0]?.isSuccess);
    if (successSnaps.length >= 2) {
      // Should return same reference (snapshot caching)
  ...
  it("returns pending state for each query config", async () => {
    const container = createTestContainer();
    container.register(UsersPort, () => ResultAsync.fromSafePromise(new Promise<any>(() => {})));
    container.register(PostsPort, () => ResultAsync.fromSafePromise(new Promise<any>(() => {})));
    ...
    function TestComponent() {
      ...
      return (
        <div>
          <span>Users: {results[0].status}</span>
          <span>Posts: {results[1].status}</span>
        </div>
      );
    }
    ...
    expect(screen.getByText("Users: pending")).toBeDefined();
    expect(screen.getByText("Posts: pending")).toBeDefined();
