...
describe("useSuspenseQuery (mutation killers)", () => {
  ...
  it("success state has correct isFetching and isRefetching fields", async () => {
    const client = createSuccessClient();
    await client.fetchQuery(UsersPort, undefined);
  ...
  it("clears suspense cache entry on success", async () => {
    const container = createTestContainer();
    container.register(UsersPort, () =>
      ResultAsync.fromSafePromise(
        new Promise<User[]>(resolve => setTimeout(() => resolve([{ id: "1", name: "Alice" }]), 30))
      )
    );
    const client = createQueryClient({ container, defaults: { retry: 0 } });
  ...
  it("sets error result in suspense entry when fetch fails", async () => {
    const container = createTestContainer();
    container.register(UsersPort, () => ResultAsync.err({ _tag: "FetchFail", message: "net" }));
    const client = createQueryClient({ container, defaults: { retry: 0 } });
    ...
    render(
      <Wrapper client={client}>
        <TestErrorBoundary fallback={e => <div>Error caught: {typeof e}</div>}>
          <Suspense fallback={<div>Suspending...</div>}>
            <TestComponent />
          </Suspense>
        </TestErrorBoundary>
  ...
  it("returns dataUpdatedAt from observer state", async () => {
    const client = createSuccessClient();
    await client.fetchQuery(UsersPort, undefined);

    let capturedDataUpdatedAt: number | undefined;
    function TestComponent() {
      const state = useSuspenseQuery(UsersPort, undefined);
      capturedDataUpdatedAt = state.dataUpdatedAt;
      return <div>Data: {state.data[0].name}</div>;
    }
    ...
    await waitFor(() => expect(screen.getByText("Data: Alice")).toBeDefined());
    expect(capturedDataUpdatedAt).toBeDefined();
    expect(typeof capturedDataUpdatedAt).toBe("number");
