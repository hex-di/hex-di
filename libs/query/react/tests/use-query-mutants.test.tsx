...
describe("useQuery dependency resolution (mutation killers)", () => {
  ...
  it("createPendingState: all boolean fields have correct values while deps pending", () => {
    const container = createTestContainer();
    container.register(ParentPort, () =>
      ResultAsync.fromSafePromise(new Promise<string>(() => {}))
    );
    ...
    // Verify every field of createPendingState
    expect(capturedState.status).toBe("pending");
    expect(capturedState.fetchStatus).toBe("idle");
    expect(capturedState.isPending).toBe(true);
    expect(capturedState.isSuccess).toBe(false);
    expect(capturedState.isError).toBe(false);
    expect(capturedState.isFetching).toBe(false);
    expect(capturedState.isRefetching).toBe(false);
    expect(capturedState.isLoading).toBe(false);
    expect(capturedState.isStale).toBe(true);
    expect(capturedState.isPlaceholderData).toBe(false);
    expect(capturedState.isPaused).toBe(false);
    expect(capturedState.result).toBeUndefined();
    expect(capturedState.data).toBeUndefined();
    expect(capturedState.error).toBeNull();
    expect(capturedState.dataUpdatedAt).toBeUndefined();
    expect(capturedState.errorUpdatedAt).toBeUndefined();
    expect(typeof capturedState.refetch).toBe("function");

    client.dispose();
  });

  it("createPendingState refetch returns a ResultAsync", async () => {
    const container = createTestContainer();
    container.register(ParentPort, () =>
      ResultAsync.fromSafePromise(new Promise<string>(() => {}))
    );
    ...
    // refetch should be callable and return something awaitable
    const result = capturedRefetch();
...
// =============================================================================
// Success state field completeness
// =============================================================================

describe("useQuery success state completeness (mutation killers)", () => {
  it("success state has all boolean fields correct", async () => {
    const { client } = createClient();
    ...
    expect(capturedState.isFetching).toBe(false);
    expect(capturedState.isRefetching).toBe(false);
    expect(capturedState.isStale).toBeDefined();
    expect(capturedState.isPlaceholderData).toBe(false);
  ...
  it("error state has all boolean fields correct", async () => {
    const client = createErrorClient();
...
// =============================================================================
// refetchInterval with false and 0
// =============================================================================

describe("useQuery refetchInterval false and 0 (mutation killers)", () => {
  it("does not refetch when interval is false", async () => {
    let fetchCount = 0;
    const container = createTestContainer();
    container.register(UsersPort, () => {
...
describe("useQuery snapshot caching (mutation killers)", () => {
  it("returns same reference on consecutive getSnapshot calls", async () => {
    const { client } = createClient();
    ...
    // Find two consecutive success snapshots
    const successSnaps = snapshots.filter((s: any) => s.isSuccess);
    if (successSnaps.length >= 2) {
      expect(successSnaps[successSnaps.length - 1]).toBe(successSnaps[successSnaps.length - 2]);
    }
