/**
 * Fetch adapter integration tests.
 *
 * Verifies that a mock HttpClient composes with combinators in the
 * same way a real fetch-based adapter would.
 */
...
describe("fetch adapter integration", () => {
  ...
  it("adapter executes GET request and returns Ok(HttpResponse)", async () => {
    ...
    const client = createMockHttpClient((req) => {
      capturedAuthHeader = req.headers.entries["authorization"];
      return ok(mockJsonResponse(200, { data: "test" }));
    });

    const composed = bearerAuth("my-token")(baseUrl("https://api.test.com")(client));
    const result = await composed.get("/api/data");
  ...
  it("adapter maps network errors to HttpRequestError Transport", async () => {
    const client = createMockHttpClient((req) =>
      err(mockRequestError("Transport", "Failed to fetch", req)),
    );

    const composed = interceptor({
      onRequest: (req) => req,
    })(baseUrl("https://api.test.com")(client));
  ...
  it("adapter maps timeout to HttpRequestError Timeout", async () => {
    const client = createMockHttpClient((req) =>
      err(mockRequestError("Timeout", "Request timeout", req)),
    );

    const result = await client.get("https://api.test.com/slow");
  ...
  it("adapter maps abort to HttpRequestError Aborted", async () => {
    const client = createMockHttpClient((req) =>
      err(mockRequestError("Aborted", "The user aborted a request", req)),
    );

    const result = await client.get("https://api.test.com/cancelled");
