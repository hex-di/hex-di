...
import type { HostClockBridge, HostBridgeClockOptions, HostBridgeClockStartupError } from "../src/adapters/host-bridge-clock.js";
...
describe("createHostBridgeClock return type", () => {
  it("createHostBridgeClock returns Result<ClockService & ClockDiagnosticsService, HostBridgeClockStartupError>", () => {
    ...
    expectTypeOf(createHostBridgeClock(bridge, { adapterName: "T", platform: "unknown" }))
      .toEqualTypeOf<Result<ClockService & ClockDiagnosticsService, HostBridgeClockStartupError>>();
  });
});
