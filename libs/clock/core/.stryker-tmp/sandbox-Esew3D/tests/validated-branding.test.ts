...
describe("asWallClockValidated()", () => {
  ...
  it("asWallClockValidated(Date.now() + 86400001) returns Err(BrandingValidationError) (>1 day in future)", () => {
    const result = asWallClockValidated(Date.now() + 86400001);
    expect(result.isErr()).toBe(true);
    if (result.isErr()) {
      expect(result.error.expectedDomain).toBe("wallClock");
    }
...
describe("asHighResValidated()", () => {
  ...
  it("asHighResValidated applies same validation rules as asWallClockValidated (Y2K floor, 1-day-future ceiling)", () => {
    ...
    // 1 day in future ceiling
    const errFuture = asHighResValidated(Date.now() + 86400001);
