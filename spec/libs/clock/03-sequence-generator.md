...
REQUIREMENT (CLK-SEQ-001): `next()` MUST be thread-safe within a single JavaScript execution context. Concurrent microtask interleaving MUST NOT produce duplicate sequence numbers.
...
REQUIREMENT (CLK-SEQ-002): `SequenceOverflowError` MUST be constructed via a factory function and frozen with `Object.freeze()` to satisfy GxP error immutability requirements.
...
REQUIREMENT (CLK-SEQ-003): When `next()` returns `err(SequenceOverflowError)`, the generator MUST remain in the overflow state permanently. All subsequent `next()` calls MUST return the same `err(SequenceOverflowError)`. The generator MUST NOT silently wrap around, return `ok(NaN)`, or return `ok()` with the same value twice.

REQUIREMENT (CLK-SEQ-004): `current()` MUST continue to return `Number.MAX_SAFE_INTEGER` after overflow.
...
**Monitoring guidance (GxP: 21 CFR 11.10(e)):** Although overflow requires ~285,000 years at sustained maximum throughput, GxP validation requires documented handling of all error paths. GxP consumers MUST implement a capacity monitoring mechanism; non-GxP consumers SHOULD implement one as a best practice:
...
The sequence generator's capacity is bounded by `Number.MAX_SAFE_INTEGER` (2^53 - 1 = 9,007,199,254,740,991). The following formulas help GxP organizations estimate time-to-overflow for their deployment.

**Time-to-overflow formula:**

```
timeToOverflowYears = (MAX_SAFE_INTEGER - currentSequence) / (callsPerSecond × 86400 × 365.25)
```

**Reference capacity table:**

| Throughput (calls/sec) | Time to Overflow | Typical Workload |
|---|---|---|
| 1 | ~285,616,414 years | Low-volume batch processing |
| 100 | ~2,856,164 years | Moderate API traffic |
| 1,000 | ~285,616 years | High-frequency transaction logging |
| 10,000 | ~28,562 years | Event streaming, IoT telemetry |
| 100,000 | ~2,856 years | Ultra-high-frequency trading audit |
| 1,000,000 | ~285 years | Theoretical maximum sustained load |

**GxP capacity monitoring threshold recommendation:**

| Deployment Criticality | Warning Threshold | Alert Threshold |
|---|---|---|
| GxP-critical (pharmaceutical, clinical) | 80% of MAX_SAFE_INTEGER | 90% of MAX_SAFE_INTEGER |
| Supporting GxP (non-critical audit) | 90% of MAX_SAFE_INTEGER | 95% of MAX_SAFE_INTEGER |
| Non-GxP | 95% of MAX_SAFE_INTEGER | 99% of MAX_SAFE_INTEGER |

REQUIREMENT (CLK-SEQ-006): GxP consumers MUST configure capacity monitoring thresholds commensurate with their deployment criticality and document the chosen thresholds in their CSVP. The thresholds above are recommendations; organizations MAY adjust based on their risk assessment.

REQUIREMENT (CLK-SEQ-005): When `next()` returns `err(SequenceOverflowError)`, the error MUST include the `lastValue` field set to `Number.MAX_SAFE_INTEGER`, enabling consumers to distinguish overflow from other error types for audit trail logging.
...
REQUIREMENT (CLK-ORD-001): When both a sequence number and a monotonic timestamp are captured for the same event (e.g., an HTTP audit history entry), the `sequenceNumber` MUST be the authoritative ordering mechanism. The monotonic timestamp is supplementary metadata.
...
REQUIREMENT (CLK-MPC-001) [OPERATIONAL]: Horizontally scaled GxP consumers MUST include a process-unique identifier alongside the `TemporalContext` in every audit entry to disambiguate events from different generator instances.
...
REQUIREMENT (CLK-MPC-002): The process-unique identifier MUST be captured once at process startup and reused for the entire process lifetime. Generating a new identifier per-request would defeat the purpose of correlating events within a process.

**Exported utility:** The following utility function generates a process instance identifier suitable for GxP audit trail disambiguation. This function is exported by `@hex-di/clock` as a first-class utility, ensuring consistent implementation across all consumers and eliminating the risk of incorrect consumer-side implementations:
 ...
 *
 * Format: `{hostname}-{startupTimestamp}-{uuid}` for human-readable traceability
 * combined with cryptographic uniqueness.
 */
function createProcessInstanceId(): string {
  const hostname = typeof os !== "undefined" ? os.hostname() : "unknown";
  const timestamp = Date.now();
  const uuid = crypto.randomUUID();
  return `${hostname}-${timestamp}-${uuid}`;
}

// Usage: capture once at startup
const PROCESS_INSTANCE_ID = createProcessInstanceId();
```

This pattern provides: (a) human-readable hostname for operational debugging, (b) startup timestamp for temporal correlation with system logs, and (c) UUID for cryptographic uniqueness across all possible collisions. The composite format is preferred over UUID-only for GxP environments because auditors can visually correlate process instances with infrastructure records during inspection.

REQUIREMENT (CLK-MPC-007): `createProcessInstanceId()` MUST be exported from the `@hex-di/clock` main entry point as a value-level function.

REQUIREMENT (CLK-MPC-008): `createProcessInstanceId()` MUST return a frozen string value. On platforms where `os.hostname()` is not available (browsers, edge runtimes), `hostname` MUST be replaced with `"unknown"`. On platforms where `crypto.randomUUID()` is not available or throws, the implementation MUST fall back to a composite identifier using `Date.now()` with microsecond precision (via `performance.now()` fractional digits) plus an internal monotonic counter to ensure uniqueness within the same microsecond: `{hostname}-{timestamp}-fallback-{performanceNow}-{counter}`. The fallback MUST NOT silently return a non-unique identifier.

REQUIREMENT (CLK-MPC-009): The returned identifier format MUST be `{hostname}-{startupTimestamp}-{uuid}`. The format is a stable contract — consumers and audit trail reconstruction tools MAY parse the components for operational diagnostics.
...
REQUIREMENT (CLK-MPC-003): Consumers MUST use the composite key `(processInstanceId, sequenceNumber)` for globally unique event ordering across process lifetimes. The `processInstanceId` disambiguates events from different process lifetimes, while the `sequenceNumber` provides total ordering within a single process lifetime.
...
The full worked example — including a 3-process scenario with crash/restart, raw audit entries, reconstruction code, and reconstructed timeline table — SHOULD be maintained in the ecosystem's GxP validation plan documentation (appendix "Validated Multi-Process Audit Trail Example").

REQUIREMENT (CLK-MPC-005) [OPERATIONAL]: GxP organizations MUST implement and validate an audit trail reconstruction procedure equivalent to the algorithm described above.

REQUIREMENT (CLK-MPC-006) [OPERATIONAL]: The reconstruction procedure MUST be documented in the computerized system validation plan and tested with representative data volumes during PQ.
