...
_Previous: [18c - Session, Auth & Extended Controls](./18c-session-auth-extended.md) | Next: [20 - HTTP Transport Validation](./20-http-transport-validation.md)_

---

This document specifies the HTTP audit bridge subsystem of `@hex-di/http-client`. It covers HTTP operation audit entries, user attribution for outbound requests, role-based access control for HTTP operations, authentication failure auditing, clock synchronization for HTTP timestamps, and cross-correlation between authorization evaluation records and HTTP operation records.

These sections build upon the port-based GxP architecture (§79 of this spec), the HTTP transport security combinators (sections 84-90), and the HTTP client's own port definitions (`HttpAuditTrailPort`, `HttpSubjectProviderPort`, `HttpClockSourcePort`, `HttpAuthorizationPort`, `HttpSignatureServicePort`).

### Subsystem Overview: Audit Bridge (§91-§97)

```
┌──────────────────────────────────────────────────────────────────┐
│                     Audit Bridge Subsystem                        │
│                                                                   │
│  §91 HttpAuditTrailPort ──── append-only, confirm(), WAL         │
│    │                                                              │
│    ├── §92 HttpOperationAuditEntry ── fields, reason, hash chain │
│    ├── §93 Subject Attribution ─────── withSubjectAttribution()   │
│    │    └── §93a Electronic Signatures ── withElectronicSignature │
│    │         └── §93b Signature Display ── rendering format       │
│    ├── §94 RBAC ──────────────────────── withHttpGuard(), SoD     │
│    ├── §95 Auth Failure Audit ─────────── withAuthFailureAudit()  │
│    ├── §96 Clock Synchronization ──────── shared ClockSource      │
│    └── §97 Cross-Correlation ──────────── withHttpAuditBridge()   │
│         └── evaluationId bidirectional linkage                    │
│                                                                   │
│  Regulatory: 21 CFR 11.10(d)(e)(g), 11.50-11.300, Annex 11 §9  │
└──────────────────────────────────────────────────────────────────┘
```

This subsystem forms the middle layers of the GxP combinator pipeline. It ensures every HTTP operation is attributed to a subject, authorized by policy, and recorded in a tamper-evident audit trail that cross-correlates with authorization decisions.
...
The authorization port (`HttpAuthorizationPort`) produces evaluation records for access control decisions. The HTTP client produces request/response lifecycle events. In GxP environments, these two audit streams must be **correlated** so that auditors can trace from an authorization decision to the HTTP operations it authorized, and vice versa.

```
Authorization Port        HTTP Operation           Audit Trail
─────────────────         ──────────────           ──────────────
...
1. **Entry-level correlation:** Each `HttpOperationAuditEntry` carries the `evaluationId` from the authorization decision that authorized the operation. This enables forward tracing (decision → HTTP operation).

2. **Request-level correlation:** Each `HttpOperationAuditEntry` carries a unique `requestId`. The authorization `AuditEntry` can optionally carry the `requestId` of the HTTP operation it authorized. This enables reverse tracing (HTTP operation → decision).
 ...
 * Follows an append-only behavioral contract:
...
REQUIREMENT: HttpAuditTrailPort implementations MUST satisfy the same behavioral
             contract as AuditTrailPort (§91 of this spec): append-only semantics, atomic
             ...
             HTTP operation entries MUST be independent of the authorization decision audit
...
```
RECOMMENDED: Organizations SHOULD use the same backing store for both authorization decision
...
```
REQUIREMENT: When gxp is true, implementations MUST support automated periodic hash
             chain integrity verification. The verifyAuditChain() operation MUST be
             executable on a configurable schedule (default: every 24 hours) to detect
             external tampering of stored audit entries (e.g., direct database
             modification bypassing the HttpAuditTrailPort interface). A hash chain
             verification failure MUST produce an HttpClientConfigurationAuditEntry
             with configurationKey "AUDIT_CHAIN_INTEGRITY_FAILURE" and severity
             CRITICAL, including: (1) the first sequence number where the chain
             breaks, (2) the expected previousHash vs. actual previousHash, and
             (3) the timestamp of detection. Verification failures MUST trigger an
             immediate operational alert and MUST be investigated as a potential
             data integrity incident per §83c. The periodic verification schedule
             and its results MUST be documented during periodic review (§83b).
             Reference: 21 CFR 11.10(e), ALCOA+ Original, EU GMP Annex 11 §9.
```

```
REQUIREMENT: The periodic hash chain verification schedule MUST be configurable via
             the hashChainVerificationInterval parameter on the HttpAuditTrailPort
             policy configuration (see Appendix F, parameter #33). The default
             interval is 86,400,000ms (24 hours). The interval MUST NOT exceed
             604,800,000ms (7 days). Setting the interval below 3,600,000ms (1 hour)
             is RECOMMENDED only for high-risk Category 1 endpoints. The configured
             interval MUST be recorded in the Validation Plan (§83a) and verified
             during IQ (§99a). Changes to the verification interval MUST follow the
             change control process (§116) and produce an
             HttpClientConfigurationAuditEntry (§88).
...
REQUIREMENT: HttpAuditTrailPort.record() follows the same synchronous return contract
             as AuditTrailPort.record() (§91 of this spec): the method MUST return
             Result<void, AuditTrailWriteError> synchronously. For network-backed
             implementations, the buffer-flush pattern from §91 of this spec applies — the
             ...
             true and the adapter uses buffered writes, WAL crash recovery (§91 of this spec)
...
REQUIREMENT: When `gxp: true` is set in the HTTP client configuration, the
             ...
             in §91 of this spec.5. The WAL ensures that audit entries survive
...
interface HttpOperationAuditEntry {
  ...
  /** Correlation to the authorization evaluation that authorized this operation. */
  ...
  /** Identity of the subject who initiated the operation (from HttpSubjectProviderPort). */
  ...
  /** Scope identifier from the evaluation context. */
  readonly scopeId: string;
  /** Per-scope sequence number for ordering (same pattern as §91 of this spec.4a). */
  readonly sequenceNumber: number;
  /** Hash chain integrity hash (same algorithm as §91 of this spec.4). */
  ...
  /**
   * Certificate revocation check result for this HTTPS connection.
   *
   * Populated by the transport layer when CertificateRevocationPolicy
   * is enabled (§106). Records the revocation check method, status,
   * timestamps, cached flag, certificate serial, and issuer CN.
   *
   * undefined when revocation checking is disabled or when the
   * connection uses plain HTTP (non-TLS). When present, the
   * `status` field MUST be included in the hash chain computation.
   *
   * Reference: §106 (CertificateRevocationResult), 21 CFR 11.10(e).
   */
  readonly revocationCheckResult: CertificateRevocationResult | undefined;
}

> **Note on `responseTimestamp` empty string (ADR #46):** An empty string `""` is a valid sentinel value for `responseTimestamp` when the HTTP operation fails at the transport level (e.g., connection refused, DNS resolution failure, timeout) and no response is received. ISO 8601 format validation MUST NOT be applied to an empty `responseTimestamp`. The hash chain computation (§91 of this spec.4) MUST include the empty string as-is — it MUST NOT be replaced with a placeholder or omitted from the hash input.

/**
 * GxP-enforced variant of HttpOperationAuditEntry.
 * Makes integrity fields non-optional at the type level.
 * Mirrors the GxPAuditEntry pattern from §91 of this spec.
 */
...
REQUIREMENT: When gxp is true, HttpAuditTrailPort adapters MUST use
             ...
             from §91 of this spec — non-GxP adapters MAY leave integrity fields as empty
...
REQUIREMENT: The requestDigest and responseDigest fields MUST be populated when the
             ...
             authorized by the authorization evaluation.
             Reference: 21 CFR 11.10(c), ALCOA+ Accurate.
```

```
REQUIREMENT: The revocationCheckResult field in HttpOperationAuditEntry MUST be
             populated when CertificateRevocationPolicy (§106) is enabled and the
             connection uses HTTPS/TLS. The field records the CertificateRevocationResult
             for the HTTPS connection, including the method used, status, timestamps,
             cached flag, certificate serial, and issuer CN. When revocation checking
             is disabled or the connection is not TLS, the field MUST be undefined.
             When present, the revocationCheckResult.status value MUST be included
             in the hash chain computation (appended after the externalCorrelationId
             in the hash input). This ensures that revocation check evidence is
             tamper-evident and auditable post-hoc.
             Reference: §106 (CertificateRevocationResult), 21 CFR 11.10(e),
             ALCOA+ Complete.
```
...
interface GxPActiveRequest {
  ...
  /** The authenticated subject from the HttpSubjectProviderPort. */
  readonly subjectId: string;
  /** Authentication method from the subject provider (the HttpSubjectProviderPort contract). */
  readonly authenticationMethod: string;
  /** Timestamp when the subject was authenticated (ISO 8601 UTC). */
  readonly authenticatedAt: string;
  /** The authorization evaluationId that authorized this request. */
  readonly evaluationId: string;
}
...
1. Before each request, resolves the current subject from the `SubjectProviderPort` (the HttpSubjectProviderPort contract)
...
REQUIREMENT: Every HTTP operation on GxP data MUST be attributable to an authenticated
             ...
             from the scoped SubjectProviderPort (the HttpSubjectProviderPort contract) and attach the subjectId
...
REQUIREMENT: The subject identity attached to the HTTP operation MUST match the
             subject identity in the authorization evaluation that authorized the operation.
...
RECOMMENDED: Organizations SHOULD implement a periodic attribution consistency check
             that compares subjectId values between authorization AuditEntry records and
             ...
             classification matrix (the incident classification framework (§83c)).
```
...
21 CFR Part 11 Subpart C (§11.50, §11.70, §11.100, §11.200, §11.300) requires that electronic signatures be unique to one individual, include at least two distinct identification components, and be linked to their respective electronic records with the signer's identity, date/time, and meaning. Section 11.70 requires signature-to-record linking. Section 11.300 requires controls for identification codes and passwords used as signature components, including unique issuance, periodic recall/revision, loss management, and unauthorized use detection. While the `HttpSignatureServicePort` manages electronic signature lifecycle (via an ecosystem adapter), HTTP operations that create, modify, or approve regulated records require a **transport-level bridge** that binds signatures to specific HTTP operations.
...
> **Biometric Scope Exclusion:** This specification addresses **non-biometric electronic signatures** per 21 CFR 11.200(a)(1)(i), which require at least two distinct identification components (e.g., user ID + password, user ID + hardware token). Biometric identification components per 21 CFR 11.200(a)(1)(ii) (e.g., fingerprint, retinal scan) are **out of scope** and would require additional specification covering biometric device qualification, template storage, false acceptance/rejection rates, and liveness detection. Organizations adopting biometric authentication MUST extend this specification with a biometric-specific addendum before GxP deployment.
...
interface HttpElectronicSignature {
  ...
  /** The authorization evaluation that captured the signature evidence. */
  readonly evaluationId: string;
  /**
...
interface ElectronicSignatureConfig {
   ...
   * `SignatureServicePort` (the HttpSignatureServicePort contract) is temporarily unreachable,
...
```
REQUIREMENT: Organizations using electronic signatures as defined in §93a MUST
             implement the following 21 CFR 11.300 controls for identification codes
             and passwords used as signature components:
             (a) Each identification code/password combination MUST be unique — no two
             individuals share the same combination of identification components.
             (b) Identification codes and passwords MUST be periodically checked,
             recalled, or revised (e.g., password expiration policies). The
             withAuthenticationPolicy() combinator (§90) enforces maxSessionAge
             as a compensating control for periodic credential revision.
             (c) Loss management procedures MUST ensure that lost, stolen, or
             compromised identification codes are immediately deactivated. The
             HttpSignatureServicePort adapter MUST support signer status revocation
             (§107 checkSignerStatus). The withTokenLifecycle() combinator (§90)
             MUST reject revoked tokens.
             (d) Unauthorized use detection MUST be implemented through audit trail
             monitoring. AuthenticationFailureAuditEntry (§95) records capture
             failed authentication attempts; organizations MUST monitor these for
             patterns indicating unauthorized use (see §95 RECOMMENDED threshold).
             (e) Transaction safeguards MUST prevent unauthorized use of electronic
             signatures. The withElectronicSignature() combinator provides
             signerId-to-subjectId matching (§93a), two-factor verification,
             and signature binding as transaction safeguards.
             Reference: 21 CFR 11.300(a)(b)(c)(d)(e).
```
...
### Connection to HttpSignatureServicePort

The `captureSignature` callback in `ElectronicSignatureConfig` is the bridge point to the `HttpSignatureServicePort`. A typical implementation using an ecosystem adapter:
...
const config: ElectronicSignatureConfig = {
  ...
  captureSignature: async (request, subject, meaning) => {
    // Delegates to HttpSignatureServicePort adapter
...
### Custom Display Layout Extension

Organizations MAY extend the `HttpSignatureDisplayFormat` with a `"custom"` layout variant to satisfy site-specific regulatory or branding requirements. The custom layout MUST still include all five mandatory manifestation fields (signerId, signerRole, signedAt, signatureMeaning, signatureBinding) per 21 CFR 11.50(a).

```typescript
/**
 * Extended display format supporting organization-specific layouts.
 *
 * The "custom" layout enables organizations to define site-specific
 * rendering templates while maintaining compliance with mandatory
 * manifestation field requirements.
 */
interface HttpSignatureDisplayFormatExtended extends HttpSignatureDisplayFormat {
  readonly layout: "inline" | "block" | "badge" | "custom";
  /**
   * Custom rendering template identifier. REQUIRED when layout is "custom".
   * References a site-specific template registered in the organization's
   * audit review interface configuration.
   *
   * Example: "pharma-batch-signature-v2", "clinical-submission-signature"
   */
  readonly customTemplateId?: string;
  /**
   * Additional metadata fields to include in the display.
   * Organizations MAY add fields beyond the five mandatory fields
   * (e.g., department, study ID, batch number) for operational context.
   * Additional fields MUST NOT replace or obscure mandatory fields.
   */
  readonly additionalFields?: ReadonlyArray<{
...
```
RECOMMENDED: Organizations using the "custom" layout extension SHOULD validate
             their custom templates during OQ to confirm that all five mandatory
             manifestation fields (signerId, signerRole, signedAt, signatureMeaning,
             signatureBinding) are rendered in a clear and unambiguous manner per
             21 CFR 11.50(a) and EU GMP Annex 11 §14. Custom templates SHOULD be
             maintained under document control and reviewed during periodic
             compliance reviews (§83b).
             Reference: 21 CFR 11.50(a), EU GMP Annex 11 §14.
```
...
interface HttpOperationPolicy {
  ...
  /** The authorization policy to evaluate (via HttpAuthorizationPort). */
...
REQUIREMENT: When gxp is true, HTTP operations that transmit or modify GxP data
             ...
             the transport adapter. Denied operations MUST produce an audit entry
             recording the denial (via HttpAuditTrailPort, §91).
...
REQUIREMENT: HttpOperationPolicy definitions MUST be JSON-serializable plain data
             (same constraint as ecosystem policy types). The policy, method,
...
interface AuthenticationFailureAuditEntry {
  ...
  /** Scope identifier. */
  readonly scopeId: string;
  /** Correlation to authorization evaluation if available. */
  readonly evaluationId: string;
...
```
REQUIREMENT: HTTP audit chains (HttpAuditTrailPort) and authorization audit chains
             (from upstream authorization systems) are independent chain namespaces.
             They do NOT share scopeId values and MUST NOT be interleaved.
             verifyAuditChain() operates on a single chain type at a time — it
             validates either authorization audit entries or HTTP audit entries, not
             both mixed together. The evaluationId field provides the cross-reference
             between HTTP audit entries and their corresponding authorization audit
             entries when a single HTTP request triggers an authorization evaluation.
...
All timestamps in `HttpOperationAuditEntry` and `AuthenticationFailureAuditEntry` MUST use the `HttpClockSourcePort` (§96). This ensures temporal consistency between authorization evaluation timestamps and HTTP operation timestamps.

```
REQUIREMENT: The withHttpAuditBridge() combinator MUST use the ClockSource from the
             HttpClockSourcePort (§96) for all timestamps in HTTP audit entries. Using
             a different clock source (e.g., Date.now() directly) would create temporal
             inconsistency between authorization and HTTP audit streams,
             violating the ALCOA+ Contemporaneous and Consistent principles.
             Reference: 21 CFR 11.10(e), ALCOA+ Contemporaneous, Consistent.
```

```
REQUIREMENT: The dual-timing strategy from the HttpClockSourcePort contract (§96) applies to HTTP operations:
             ...
             Reference: §96 (Dual-Timing Strategy).
```
...
REQUIREMENT: When `gxp: true` is set, the HTTP client MUST enforce a maximum clock drift
             ...
             Reference: §96 of this spec, 21 CFR 11.10(e), ALCOA+ Contemporaneous.
```

### Clock Correction Behavior

When the system clock undergoes NTP step corrections (as opposed to gradual NTP slew corrections), the following behaviors MUST be enforced:

```
REQUIREMENT: When a wall-clock step correction >1 second is detected (either
             forward or backward), the HTTP client MUST:
             (1) Immediately pause all GxP HTTP operations,
             (2) Record a CLOCK_CORRECTION_DETECTED ConfigurationAuditEntry (§88)
                 containing: correction direction (forward/backward), magnitude in
                 milliseconds, pre-correction timestamp, post-correction timestamp,
                 and whether monotonic clock was affected,
             (3) Verify that the monotonic clock (performance.now()) was NOT affected
                 by the step correction — if monotonic timing is also disrupted,
                 escalate to CF-04 (§115.4),
             (4) Wait for clock stabilization: the wall-clock MUST produce 3
                 consecutive health checks (at 1-second intervals) showing drift
                 ≤ 1 second before operations resume,
             (5) Resume GxP operations only after stabilization is confirmed,
             (6) Record a CLOCK_CORRECTION_RESOLVED ConfigurationAuditEntry
                 documenting the stabilization timestamp and total pause duration.
             During the pause period, any pending operations in the retry queue
             MUST retain their original timestamps (not re-timestamp). Operations
             initiated during the pause MUST be queued and assigned timestamps
             only after clock stabilization.
             Reference: 21 CFR 11.10(e), ALCOA+ Contemporaneous, EU GMP Annex 11 §9.
```

| Correction Type | Detection Method | GxP Impact | Recovery |
|----------------|------------------|------------|----------|
| **NTP slew** (gradual, < 500ms/s) | Drift within threshold | None — timestamps remain accurate | No action required |
| **NTP step forward** (> 1 second) | Sudden jump in Date.now() | Operations paused; gap in timestamps | Wait for stabilization per REQUIREMENT above |
| **NTP step backward** (> 1 second) | Date.now() decreases | Operations paused; potential duplicate timestamps | Wait for stabilization; verify no duplicate sequenceNumbers |
| **Monotonic clock unaffected** | performance.now() continues normally | durationMs fields remain accurate | Only wall-clock timestamps affected |
| **Monotonic clock disrupted** | performance.now() jumps or resets | CF-04 Critical | Full clock source failure recovery per §115.4 |
...
```
REQUIREMENT: When gxp is true, the clock drift detection event MUST be recorded as an
             HttpClientConfigurationAuditEntry with configurationKey
             "CLOCK_DRIFT_DETECTED" including: (1) the measured drift in milliseconds,
             (2) the NTP source hostname or IP, (3) the timestamp of detection, and
             (4) whether HTTP operations were paused as a result. Clock drift events
             MUST be reviewable during periodic review (§83b). Organizations MUST
             document the NTP server infrastructure (primary and fallback NTP sources,
             stratum level, polling interval) in the Validation Plan (§83a) and
             verify NTP connectivity during IQ (§99a).
             Reference: 21 CFR 11.10(e), ALCOA+ Contemporaneous, EU GMP Annex 11 §9.
```

```
RECOMMENDED: For non-GxP mode, organizations SHOULD include HTTP operation timestamps
             in their NTP drift monitoring (the HttpClockSourcePort contract (§96)). If the ClockSource
...
## 97. Cross-Correlation: Authorization to HTTP

### Correlation Structure

```typescript
interface HttpAuthorizationCorrelation {
  readonly _tag: "HttpAuthorizationCorrelation";
  /** The authorization evaluation that authorized the HTTP operation. */
  readonly evaluationId: string;
  /** The HTTP operation's unique identifier. */
  readonly requestId: string;
  /** The subject who initiated both the authorization evaluation and HTTP operation. */
  readonly subjectId: string;
  /** Authorization decision timestamp (ISO 8601 UTC). */
  readonly decisionTimestamp: string;
  /** HTTP request timestamp (ISO 8601 UTC). */
  readonly requestTimestamp: string;
  /** Time elapsed between authorization decision and HTTP request in milliseconds. */
...
function withHttpAuditBridge(
  ...
  options?: {
    /** Maximum allowed time between authorization decision and HTTP request. */
...
6. Optionally validates that the authorization decision is recent enough (`decisionToRequestMs <= maxDecisionAge`)
...
REQUIREMENT: The withHttpAuditBridge() combinator MUST produce an HttpOperationAuditEntry
             ...
             completeness as required by the HttpAuditTrailPort contract (§91).
             Reference: 21 CFR 11.10(e), ALCOA+ Complete.
```

```
REQUIREMENT: The evaluationId in HttpOperationAuditEntry MUST match the evaluationId
             of the authorization AuditEntry that authorized the operation. This bidirectional
             correlation MUST be verifiable via audit trail queries: given an
             evaluationId, an auditor MUST be able to find both the authorization decision
...
REQUIREMENT: When maxDecisionAge is configured, the combinator MUST reject HTTP
             requests where the time elapsed since the authorization decision exceeds
...
```
REQUIREMENT: When gxp is true, maxDecisionAge MUST be configured
...
For auditors to trace from authorization decisions to HTTP operations and back:

**Forward trace (decision → HTTP operations):**

```
SELECT * FROM http_operation_audit
WHERE evaluationId = '<evaluationId from authorization AuditEntry>'
ORDER BY sequenceNumber;
```

**Reverse trace (HTTP operation → authorization decision):**
...
RECOMMENDED: Organizations SHOULD implement the above query patterns (or equivalent)
             as part of their audit trail review interface. The queries SHOULD be
             available through programmatic tooling for use during periodic reviews.
```
...
The Write-Ahead Log (WAL) pattern from §91 applies to HTTP audit entries. When `failOnAuditError` is true (the GxP default), an HTTP operation that cannot record its audit entry MUST fail. The WAL provides crash recovery:
...
```
REQUIREMENT: When gxp is true, HTTP audit implementations MUST use the "http:" scope
             prefix for all WAL entries to distinguish HTTP audit WAL entries from
             authorization audit WAL entries. The scope identifier format MUST be
             "http:<scopeId>" (e.g., "http:scope-uuid"). HTTP WAL recovery MUST
             only process entries with the "http:" prefix — entries without this
             prefix MUST NOT be processed by HTTP recovery to prevent
             cross-contamination between audit domains. This prefix partitioning
             ensures that HttpWalStorePort can be shared between HTTP and
             authorization audit systems without data integrity risk.
             Reference: ALCOA+ Consistent, 21 CFR 11.10(e).
```

```
RECOMMENDED: Non-GxP HTTP audit implementations SHOULD also use the "http:" scope
             prefix for WAL entries. Organizations MAY use a dedicated HttpWalStorePort
             adapter for HTTP audit entries instead of sharing with the authorization
             audit trail.
```

```
REQUIREMENT: WAL recovery after process crash MUST be atomic: either all orphaned
             HTTP audit WAL intents are recovered (re-recorded to the audit trail
             with hash chain continuity preserved) or none are recovered. Partial
             recovery where some intents are re-recorded and others are lost is
             PROHIBITED. If any single intent cannot be recovered (e.g., due to
             WAL corruption), the entire recovery MUST fail with a WalRecoveryError
             and the operator MUST be alerted with a CRITICAL severity notification.
             The failed recovery MUST be recorded as an
             HttpClientConfigurationAuditEntry with configurationKey
             "WAL_RECOVERY_FAILED" including the count of unrecoverable intents,
             the first and last affected sequence numbers, and the failure reason.
             Reference: 21 CFR 11.10(e), EU GMP Annex 11 §13, ALCOA+ Complete.
```
