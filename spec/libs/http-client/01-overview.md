...
- **Transport adapter contract** for plugging any HTTP library (fetch, axios, got, ky, ofetch, undici, or custom)
...
- Transport adapter contract with `createHttpClientAdapter` factory
...
### Adapters replace transport coupling

Traditional HTTP libraries embed transport-specific code wherever requests are made:
...
// Adapter: declares HOW to implement it (transport-specific)
...
2. **Transport agnosticism** -- same service code runs with any HTTP library or runtime
...
6. **GxP compliance** -- optional regulatory compliance for pharmaceutical, biotech, and medical device environments. See [17 - GxP Compliance](./compliance/gxp.md) for audit trails, HTTPS enforcement, credential protection, electronic signatures, and the complete regulatory mapping
...
│   │   ├── adapters/
│   │   │   ├── create-adapter.ts      # createHttpClientAdapter factory
...
│   └── tsconfig.json
│
├── fetch/                             # @hex-di/http-client-fetch
│   ├── src/
...
│   └── tsconfig.json
│
├── axios/                             # @hex-di/http-client-axios
│   ├── src/
│   │   ├── axios-client.ts            # createAxiosHttpClient implementation
│   │   ├── adapter.ts                 # AxiosHttpClientAdapter
│   │   └── index.ts
│   ├── package.json
│   └── tsconfig.json
│
├── got/                               # @hex-di/http-client-got
│   ├── src/
│   │   ├── got-client.ts              # createGotHttpClient implementation
│   │   ├── adapter.ts                 # GotHttpClientAdapter
│   │   └── index.ts
│   ├── package.json
│   └── tsconfig.json
│
├── ky/                                # @hex-di/http-client-ky
│   ├── src/
│   │   ├── ky-client.ts               # createKyHttpClient implementation
│   │   ├── adapter.ts                 # KyHttpClientAdapter
│   │   └── index.ts
│   ├── package.json
│   └── tsconfig.json
│
├── ofetch/                            # @hex-di/http-client-ofetch
│   ├── src/
│   │   ├── ofetch-client.ts           # createOfetchHttpClient implementation
│   │   ├── adapter.ts                 # OfetchHttpClientAdapter
│   │   └── index.ts
│   ├── package.json
│   └── tsconfig.json
│
...
│   │   ├── adapter.ts                 # NodeHttpClientAdapter
│   │   └── index.ts
│   ├── package.json
│   └── tsconfig.json
│
├── undici/                            # @hex-di/http-client-undici
│   ├── src/
│   │   ├── undici-client.ts           # undici-based implementation
│   │   ├── adapter.ts                 # UndiciHttpClientAdapter
│   │   └── index.ts
│   ├── package.json
│   └── tsconfig.json
│
         ...
         @hex-di/graph   @hex-di/runtime  @hex-di/result
               ...
               └──────────────┼──────────────┘
                       ...
                       @hex-di/http-client
                              │
        ┌─────────────────────┼─────────────────────┐
        │     Transport Adapters (pick one)          │ (optional)
        │                                            │     ▼
        │  ┌──────────────────────────────────┐      │  @hex-di/tracing
        │  │ @hex-di/http-client-fetch        │      │
        │  │ @hex-di/http-client-axios        │      │
        │  │ @hex-di/http-client-got          │      │
...
│  │                                                                       │  │
│  │  ┌───────┐ ┌───────┐ ┌─────┐ ┌────┐ ┌────────┐ ┌────────┐ ┌─────┐│  │
│  │  │ Fetch │ │ Axios │ │ Got │ │ Ky │ │ Ofetch │ │ Undici │ │ ... ││  │
│  │  └───────┘ └───────┘ └─────┘ └────┘ └────────┘ └────────┘ └─────┘│  │
│  └──────────────────────────────────────────────────────────────────────┘  │
│                                                                             │
├─────────────────────────────────────────────────────────────────────────────┤
...
_Previous: [00 - User Requirements Specification](./00-urs.md) | Next: [02 - Core Types](./02-core-types.md)_
