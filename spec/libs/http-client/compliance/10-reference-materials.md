# GxP Compliance — @hex-di/http-client: Reference Materials, Quick Reference & Validation Plans
...
| F-ERR-01   | EU GMP Annex 11 §13, §16              | §115         | DoD 26   | Catastrophic failure scenario tests (CF-01 through CF-06)       |
...
| F-INT-01   | GAMP 5 §5.5, EU GMP Annex 11 §4       | §118         | DoD 26   | Port contract compatibility type tests in IQ                    |
...
- Catastrophic failure recovery runbook covering 6 failure scenarios (CF-01 through CF-06) with recovery procedures (§115)
...
- Port dependency inventory with 14 port interfaces (including HttpAuditMetaEntry and HttpAuditEncryptionPort) and adapter requirements (§118)

### Test Counts

| Category          | Count | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| ----------------- | ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Unit tests        | 28    | CorsPolicy construction validation (wildcard origin rejected with credentials, exact-match enforcement), withCorsHardening() origin mismatch detection, withCorsHardening() method validation, CorsViolationError for each code (4 codes), CORS-blocked audit entry production, rate limit audit entry in GxP mode (queued delay, rejection), signing ceremony callback cancellation handling, signing ceremony re-authentication requirement, catastrophic failure: audit backend failure + WAL full blocks operations, catastrophic failure: crypto unavailable blocks operations, catastrophic failure: subject provider unavailable blocks after threshold, catastrophic failure: clock source failure blocks operations, catastrophic failure: cert infrastructure failure with hard-fail blocks, catastrophic failure recovery audit entry production, port contract compatibility check (5 ports), SemVer classification (major=full, minor=partial, patch=regression, pre-release=rejected), CORS header validation (Access-Control-Allow-Credentials), rate limit queue depth tracking, signing ceremony timeout enforcement, catastrophic failure: all 5 ports simultaneously unavailable, port adapter version compatibility, training record structure field population, training completion tracking per role, document distribution tracking |
| Type tests        | 8     | CorsPolicy discriminant, CorsViolationError codes, IamIntegrationSurface structure, SemVer revalidation mapping type, port contract compatibility types, training role enumeration, catastrophic failure mode discriminant, rate limit configuration structure                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| Integration tests | 12    | withCorsHardening + audit bridge end-to-end, CORS-blocked operation in full GxP pipeline, rate limit + audit bridge GxP recording, signing ceremony + withElectronicSignature + withSubjectAttribution combined, catastrophic failure recovery: audit backend restore + WAL flush + hash chain verification, catastrophic failure recovery: subject provider restore + operation resumption, port adapter compatibility IQ check end-to-end, full GxP pipeline with CORS hardening, SemVer upgrade + revalidation scope assessment, CR process documentation verification in periodic review, CORS + credential protection combined, catastrophic failure cascading recovery sequence                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
- [ ] Port dependency inventory lists all 14 port interfaces with spec sections
- [ ] Port adapter requirements documented for container configuration
- [ ] Port contract compatibility verified in IQ via type-level tests
...
> **Note:** Totals above are for GxP transport tests only. Additional tests in §16 include 22 E2E (counted in core), 10 chaos, 5 load, and 3 soak tests. Grand total across all sources: **272 (core §1-78, including 22 E2E) + 40 (transport adapters §44a-§44d) + 450 (GxP DoDs 20-27) + 18 (chaos/load/soak) = 780 tests**.
...
| ------------ | --------------------------------------- | --------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------- |
| **11.10(a)** | System validation                       | §83a (Validation Plan), §99 (IQ/OQ/PQ), §108 (GAMP 5 Category 5)                                                | `createGxPHttpClient` factory (§103)                                                                       | IQ-HT-01 through IQ-HT-07, all OQ checks                   |
...
| -------- | ------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------- |
| **§1**   | Risk management                 | §98 (FMEA, 43 failure modes, ICH Q9), §83c (incident classification)                                                                                             | — (risk framework)                                                                         | FMEA review in periodic review                                              |
| **§2**   | Training                        | §109 (5 roles, competency assessment)                                                                                                                            | — (organizational control)                                                                 | Training matrix review                                                      |
| **§3**   | Suppliers and service providers | §108 (supply chain classification table), §118 (port dependency inventory, 14 port interfaces documented)                                                         | — (organizational/supplier control)                                                        | Supplier assessment in periodic review                                      |
...
| **§13**  | Incident management             | §83c (incident classification, 16 types, 4 severity levels), §115 (catastrophic recovery, 6 scenarios)                                                           | — (process control)                                                                        | CF-01 through CF-06 recovery tests                                          |
...
| **§16**  | Business continuity             | §115 (catastrophic failure recovery, 6 scenarios), §104a (backup/restore)                                                                                        | — (process/operational control)                                                            | CF-01 through CF-06, backup/restore tests                                   |
...
| ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------ | ------------------------------------------------------- |
| **Attributable**    | `withSubjectAttribution()` resolves authenticated subject for every operation; `evaluationId` cross-correlation links to authorization decisions                    | §93, §97                             | `GxPActiveRequest.subjectId`, `HttpAuthorizationCorrelation`    |
...
| **Risk Assessment**          | 43-entry FMEA with S/L/D scoring; all mitigated to RPN <= 8                 | §98           |
...
| **Supplier Assessment**      | Port dependency inventory with 14 port interfaces; adapter requirements documented | §118          |
| **Periodic Review**          | Annual review schedule; 6 revalidation triggers; 8-area review scope        | §83b          |

---

## WHO TRS 996 / 1033 / MHRA DI Guidance / PIC/S PI 041

| Requirement Area            | Spec Coverage                                                              | Spec Sections                | Regulatory Source |
| --------------------------- | -------------------------------------------------------------------------- | ---------------------------- | ----------------- |
| **Data Integrity (ALCOA+)** | All 9 ALCOA+ principles addressed with specific implementations            | §91-§97, §104, §86, §93, §96 | WHO TRS 996, WHO TRS 1033 Annex 4, MHRA DI §6, PIC/S PI 041 §9 |
| **Validation Framework**    | IQ/OQ/PQ qualification with 780 specified tests                            | §99                          | WHO TRS 996, MHRA DI |
| **Traceability**            | 62-finding regulatory traceability matrix                                  | §100                         | WHO TRS 1033 Annex 4 §3.3 |
| **Audit Trail Retention**   | 5-year minimum retention; self-contained archives with integrity manifests | §104                         | WHO TRS 1033 Annex 4 §3.4, MHRA DI §6.20, PIC/S PI 041 §9.5 |
| **Audit Trail Access**      | `QueryableHttpAuditTrailPort` with 4-hour inspector access SLA             | §105                         | WHO TRS 1033 Annex 4 §3.5, MHRA DI §6.14, PIC/S PI 041 §9.6 |
| **Risk Management**         | 43-entry FMEA with ICH Q9 methodology                                      | §98                          | WHO TRS 996, PIC/S PI 041 §8 |
...
> **Cross-reference:** Full classification scheme with criteria, escalation rules, and endpoint registry guidance in [§84 — Transport Security](./04-transport-security.md).
...
> **Cross-reference:** The port dependency inventory in §118 ([09 — Advanced Requirements](./09-advanced-requirements.md)) lists the 14 port interfaces that GxP features depend on. This table extends that inventory with additional runtime ports and lifetime/requirement metadata for deployment planning.
...
| `HttpWalStorePort`              | Singleton | Yes                           | §91                   | Durable audit entry persistence (WAL)     |
...
| `HttpClockSourcePort`           | Singleton | Yes                           | §96                   | Timestamp source for audit entries        |
| `HttpOperationGatePort`         | Scoped    | Yes                           | §94                   | RBAC policy evaluation                    |
| `HttpAuthorizationPort`         | Scoped    | Yes                           | §94                   | Authorization policy evaluation           |
| `HttpSubjectProviderPort`       | Scoped    | Yes                           | §93                   | Subject identity resolution               |
| `HttpSignatureServicePort`      | Singleton | Conditional (when e-sig used) | §93a                  | Signature capture delegation              |
| `HttpSignatureVerificationPort` | Singleton | Conditional (when e-sig used) | §107                  | Signature verification                    |
| `HttpAuditRetentionPolicy`      | Singleton | Yes                           | §104                  | Pattern alignment for retention periods   |
| `HttpAuditMetaEntry`            | Singleton | Yes                           | §105                  | Meta-audit trail for audit access events  |
| `HttpAuditEncryptionPort`       | Singleton | Conditional (when encryption used) | §104c            | Encryption key lifecycle management       |
...
| **Total**                       | **550** | **75**     | **115**     | **22** | **18**          | **780** |

> **Note:** Core E2E expanded from 5 to 22 for GxP coverage (E2E-001–E2E-022). E2E-021 and E2E-022 added for configuration change control and authentication strength enforcement. Chaos/Load/Soak = 10 chaos (CF-001–CF-010) + 5 load (LT-001–LT-005) + 3 soak (SK-001–SK-003) = 18. Transport adapter tests (40) cover per-adapter body serialization and error mapping (§44a-§44d). Grand total: **272 (core) + 40 (adapters) + 450 (GxP) + 18 (chaos/load/soak) = 780 tests**.
...
| 26: Audit v5.0 Remediations   | 28      | 8      | 12          | 48      |
| 27: GxP Compliance Hardening  | 18      | 4      | 8           | 30      |
| **GxP Subtotal**              | **284** | **63** | **103**     | **450** |

> **Cross-reference:** The per-DoD breakdown above matches the authoritative table in [09 — Advanced Requirements](./09-advanced-requirements.md). Adversarial OQ test cases (OQ-HT-ADV-01 through OQ-HT-ADV-05) are counted within DoD 20. OQ checks OQ-HT-01 through OQ-HT-96 are distributed across the DoD items based on their spec section coverage.
...
## Requirements Traceability Matrix -- HTTP-Client

> For the generic RTM template, see [../../../cross-cutting/gxp/07-traceability-matrix-template.md](../../../cross-cutting/gxp/07-traceability-matrix-template.md). This section covers HTTP-client-specific traceability entries.


---

This document is the standalone regulatory traceability matrix for auditor access. It is an exact copy of §100 from [07 — Validation Protocols](./07-validation-protocols.md) extracted for independent version control and easier auditor navigation.
...
## Validation Protocols -- HTTP-Client-Specific Steps

> For the generic IQ/OQ/PQ/DQ validation protocol template, see [../../../cross-cutting/gxp/06-validation-protocol-template.md](../../../cross-cutting/gxp/06-validation-protocol-template.md). This section covers HTTP-client-specific validation protocols.
