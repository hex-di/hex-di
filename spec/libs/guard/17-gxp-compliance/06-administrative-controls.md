...
| Change History   | 1.4 (2026-02-20): Updated supplier qualification evidence test count from 713/21 → 1294/29 DoD items (CCR-GUARD-045) |
|                  | 1.3 (2026-02-13): Added Quality Reviewer role (§64g) with EU GMP Annex 11 §1 mapping, updated incompatibility matrix |
...
REQUIREMENT: When gxp is true, organizations MUST define and maintain a formal JSON
             ...
             (a) Based on the Policy discriminated union defined in 04-policy-types.md,
                 covering all ten policy kinds (hasPermission, hasRole, hasAttribute,
                 hasResourceAttribute, hasSignature, hasRelationship, allOf, anyOf, not, labeled).
...
REQUIREMENT: When gxp is true, the diff report generated by createPolicyDiffReport()
             (section 54 in 13-testing.md) MUST be archived alongside the
...
REQUIREMENT: Organizations MUST include `@hex-di/guard` in their supplier
             ...
             2. Test execution reports from the library's CI pipeline (1294 tests
                across 29 Definition of Done items — section 16)
...
- **Section 12-inspection.md (MCP Resources / A2A Skills):** The existing authentication REQUIREMENT for MCP/A2A endpoints (section 48d) is extended by the `guard:inspection:read`, `guard:audit:query`, and `guard:meta-audit:query` operation policies, which provide operation-specific authorization in addition to endpoint-level authentication.
