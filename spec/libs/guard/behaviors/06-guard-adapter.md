...
## BEH-GD-025: guard() Function (§25)

> **Invariants:**
> - [INV-GD-006](../invariants.md) — Audit Trail Completeness
> - [INV-GD-013](../invariants.md) — Guard Throws on Deny
> - [INV-GD-014](../invariants.md) — Audit Before Throw
> - [INV-GD-010](../invariants.md) — WAL Enforcement in GxP Mode
> - and others; see [invariants](../invariants.md)
> **DoD:** [DoD 7: Guard Adapter](../16-definition-of-done.md#dod-7-guard-adapter), [DoD 15: Electronic Signatures](../16-definition-of-done.md#dod-15-electronic-signatures)
 ...
 *
 * See ../compliance/gxp.md section 61 for the full behavioral contract.
 */
interface AuditTrail {
   ...
   * See ../compliance/gxp.md section 61.3a for the full durability tier specification.
   *
   ...
   * ../compliance/gxp.md section 61 for the full buffering and crash recovery guidance.
   *
 ...
 * and electronic signatures. See ../compliance/gxp.md for implementation guidance.
 */
interface AuditEntry {
  ...
  /** ISO 8601 UTC timestamp of when the decision was recorded. Must use NTP-synchronized clock in production. See [ADR #17](../decisions/017-iso-8601-timestamps.md). */
  ...
  /** Human-readable reason for the decision. Empty string for Allow decisions (see [ADR #20](../decisions/020-audit-entry-reason-duration.md)). */
  readonly reason: string;
  /** Evaluation duration in milliseconds (see [ADR #20](../decisions/020-audit-entry-reason-duration.md)). */
   ...
   * Reference: 21 CFR 11.10(c), ../compliance/gxp.md section 61.4.
   */
 ...
 *
 * See ADR #26 ([Appendix A](../appendices/architectural-decisions.md)) for rationale on the subtype approach.
 */
...
See [ADR #23](../decisions/023-optional-signature-service.md) for the rationale behind making this port optional.
 ...
 *
 * See ../compliance/gxp.md section 62 for guard-specific
...
REQUIREMENT: GxP audit trail adapters MUST enforce retention periods that meet or
             exceed the jurisdiction-specific minimums defined in ../compliance/gxp.md
 ...
 * and completeness requirements in ../compliance/gxp.md section 61.
 */
...
REQUIREMENT: When gxp is true, walStore MUST be provided to createGuardGraph().
             ...
             Reference: 21 CFR 11.10(e), EU GMP Annex 11 §9, ../compliance/gxp.md
             section 61.
```
...
## BEH-GD-026: Type Transformation (§26)

> **Invariant:** [INV-GD-015](../invariants.md) — failOnAuditError GxP Enforcement
...
### Conditional Port Detection

`guard()` scans the policy tree at construction time to detect policy kinds that require additional infrastructure ports. This scan is a one-time cost at adapter creation, not per-evaluation.

```typescript
/**
 * Walks a policy tree and returns the set of policy kinds present.
 * Used by guard() to determine which optional ports to add to requires.
 */
function detectPolicyKinds(policy: PolicyConstraint): ReadonlySet<PolicyKind> {
  const kinds = new Set<PolicyKind>();
  function walk(p: PolicyConstraint): void {
    kinds.add(p.kind);
    if (p.kind === "allOf" || p.kind === "anyOf") {
      for (const child of (p as AllOfPolicy<readonly PolicyConstraint[]>).policies) {
        walk(child);
      }
    } else if (p.kind === "not") {
      walk((p as NotPolicy<PolicyConstraint>).policy);
    }
  }
  walk(policy);
  return kinds;
...
**Policy-dependent port additions:**

| Policy Kind       | Required Port              | When Added                                                     |
|-------------------|----------------------------|----------------------------------------------------------------|
| `hasSignature`    | `SignatureServicePort`     | When any `hasSignature` node is in the policy tree             |
| `hasRelationship` | `RelationshipResolverPort` | When any `hasRelationship` node is in the policy tree          |

```
REQUIREMENT: guard() MUST scan the policy tree (resolve + all methodPolicies) at
             construction time and add RelationshipResolverPort to the adapter's
             requires when any hasRelationship policy is detected. This ensures
             compile-time and runtime dependency validation catches missing
             resolvers before the first evaluation.
             Reference: ADR #52.
```

## BEH-GD-027: guardAsync() Function (§25a)

The `guardAsync()` function is the async variant of `guard()` (see [ADR #49](../decisions/049-async-guards-singleton-lifetime.md) for the singleton lifetime constraint). It wraps an adapter with authorization enforcement that supports on-demand attribute resolution and async relationship checking via `evaluateAsync()`.

```typescript
/**
 * Wraps an adapter with async authorization enforcement.
 *
 * Like guard(), but the factory is async: it calls evaluateAsync()
 * which can resolve missing attributes on-demand via AttributeResolver.
 *
 * IMPORTANT: guardAsync() forces singleton lifetime on the resulting adapter.
 * Async factories cannot participate in scoped or transient resolution because
 * the container's resolve() is synchronous. The singleton guard resolves once
 * and caches the result.
 *
 * @typeParam A - The adapter type (must have singleton lifetime)
 * @param adapter - The adapter to wrap (singleton lifetime required)
 * @param options - Object with resolve policy and optional async configuration
 * @returns A new singleton adapter with augmented requires
 */
export function guardAsync<A extends AdapterConstraint & { lifetime: "singleton" }>(
  adapter: A,
  options: {
    readonly resolve: PolicyConstraint;
    readonly methodPolicies?: MethodPolicyMap;
    readonly resolverTimeoutMs?: number;   // Default: 5000ms
    readonly maxConcurrentResolutions?: number; // Default: 10
  }
): GuardedAsyncAdapter<A>;
```

```
REQUIREMENT: guardAsync() MUST reject adapters with non-singleton lifetime at
             compile time. The conditional type GuardedAsyncAdapter MUST produce
             a human-readable type error when the adapter's lifetime is "scoped"
             or "transient".
             Reference: ADR #49.

REQUIREMENT: guardAsync() MUST capture evaluatedAt timestamp BEFORE any async
             resolution begins. This ensures the audit trail timestamp reflects
             when the evaluation was initiated, not when resolution completed.
             Reference: ALCOA+ Contemporaneous, 21 CFR 11.10(e).
```

## BEH-GD-028: Requires Deduplication (§27)
...
## BEH-GD-029: Guard Factory Behavior (§28)
...
All WARNING and INFO log events emitted by the guard pipeline (rate limiting activation, scope expiry, clock drift, audit write failure, field truncation, WAL recovery) conform to the structured `GuardOperationalEvent` schema defined in Appendix R ([Operational Log Event Schema](../appendices/operational-log-schema.md)). This schema provides consistent event typing for SIEM ingestion and alerting.
...
REQUIREMENT: Emergency access events MUST be flagged for mandatory review within
             24 hours per the incident classification matrix (../compliance/gxp.md
...
RECOMMENDED: Organizations SHOULD document their break-glass procedure in the
             validation plan (../compliance/gxp.md section 67), including: (a) the
...
RECOMMENDED: Implement time-based access restrictions using hasAttribute policies
             ...
             ClockSource (../compliance/gxp.md section 62) to ensure temporal
...
REQUIREMENT: When gxp is true, verifyAuditChain() MUST be invoked at scope
             ...
             in section 61.4 (../compliance/gxp.md). The verification MUST NOT
 ...
 *     escalates from warn to fail ([ADR #44](../decisions/044-memory-audit-trail-production-escalation.md)). In non-production environments,
 ...
 * 14. When gxp is true, verifies that maxRecentDecisions >= 200 entries.
 *     Emits warn if below 200 (insufficient overlap between operational
 *     monitoring poll intervals and evaluation throughput).
 *     Reference: 12-inspection.md, EU GMP Annex 11 §9, ALCOA+ Available.
 * 15. When gxp is true, verifies that predicateRuleMapping is present and
 *     non-empty. Emits fail with diagnostic code
 *     "guard.predicate-rule-mapping-missing" if missing or empty.
 *     Reference: 01-regulatory-context.md §59, REQ-GUARD-067.
 *
...
_Next: [08 - Port Gate Hook](./07-port-gate-hook.md)_
