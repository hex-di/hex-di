...
> | Change History   | 1.5 (2026-02-20): Corrected note at end of §57: Vitest count 917 → 1176, total 1035 → 1294 (CCR-GUARD-045) |
> |                  | 1.3 (2026-02-15): Elevated test data version control from RECOMMENDED to REQUIREMENT when gxp:true, requiring git SHA reference in OQ/PQ reports (CCR-GUARD-016) |
> |                  | 1.1 (2026-02-19): Added BEH-GD-NNN requirement identifiers to section headings (CCR-GUARD-020) |
> |                  | 1.2 (2026-02-14): Added Cucumber BDD features for ReBAC (3 files, 14 scenarios), async evaluation (2 files, 8 scenarios), field union (2 files, 7 scenarios), cross-enhancement (1 file, 4 scenarios). Updated GuardCucumberWorld with attributeResolver and relationshipResolver. Total: 23 feature files, 118 scenarios. (CCR-GUARD-004) |
> |                  | 1.1 (2026-02-14): Added section 57 (Cucumber BDD Acceptance Tests) with runner configuration, World class, step definitions, tag taxonomy, 15 feature files, ~85 scenarios (CCR-GUARD-003) |
> |                  | 1.0 (2026-02-13): Initial controlled release |

_Previous: [12 - Inspection](./11-inspection.md)_

---

## BEH-GD-054: Memory Adapters (§49)

> **Invariant:** [INV-GD-006](../invariants.md) — Audit Trail Completeness (memory audit trail adapter)
> **DoD:** [DoD 19: Testing Infrastructure](../16-definition-of-done.md#dod-19-testing-infrastructure)

Following the established patterns of `MemoryTracer` and `MemoryLogger`, memory adapters implement the production interface while exposing inspection methods for test assertions (see [ADR-GD-011](../decisions/011-separate-guard-testing-package.md) for the rationale behind the separate `@hex-di/guard-testing` package). All use the `create*` factory convention.
 ...
 * requirements in ../compliance/gxp.md section 61.
 *
 * @param options - Optional configuration
...
A reusable conformance test suite that validates any `AuditTrailPort` adapter against the GxP behavioral contract (../compliance/gxp.md section 61). Adapter authors run this suite as part of their adapter's OQ evidence to prove conformance without re-implementing the same test logic.
 ...
 * AuditTrailPort contract (../compliance/gxp.md section 61):
...
REQUIREMENT: All three reference adapters (MemoryAuditTrail,
             ...
             (../compliance/gxp.md section 61) is correctly implemented and that
...
A reusable conformance test suite that validates any `SignatureService` adapter against the behavioral contract (07-guard-adapter.md section 25, ../compliance/gxp.md sections 65a-65d). Adapter authors run this suite as part of their adapter's OQ evidence to prove conformance without re-implementing the same test logic.
 ...
 * (../compliance/gxp.md sections 65a-65d):
...
A reusable conformance test suite that validates administrative guard operations against the behavioral contract defined in ../compliance/gxp.md sections 64a-64g. This suite validates the `AdminGuardConfig` deny-by-default enforcement, administrative role authorization, policy change control, and separation of duties for administrative operations.
 ...
 * controls contract (../compliance/gxp.md sections 64a-64g):
...
## BEH-GD-056: Subject Fixtures (§51)
...
## BEH-GD-057: testPolicy and testGuard (§52)

> **Invariant:** [INV-GD-012](../invariants.md) — Evaluation Determinism
...
## BEH-GD-058: Anti-Patterns (§53)

> **Invariant:** [INV-GD-013](../invariants.md) — Guard Throws on Deny (anti-pattern: catching AccessDeniedError)
...
## BEH-GD-059: Policy Change Testing Utilities (§54)
...
REQUIREMENT: In GxP environments, createPolicyDiffReport() MUST be run before
             ...
             process (../compliance/gxp.md section 64a). The report MUST be reviewed
...
## BEH-GD-060: Test Data Management for GxP (§55)
...
REQUIREMENT: Test datasets MUST NOT contain production data (real patient identifiers,
             ...
             created using createTestSubject() (section 51) or equivalent synthetic
...
REQUIREMENT: Test audit entries MUST be distinguishable from production audit entries.
             ...
             MUST be documented in the validation plan (../compliance/gxp.md section 67).
             Reference: GAMP 5 Appendix D4.
```

```
REQUIREMENT: When gxp is true, test data sets used for Operational Qualification (OQ)
             and Performance Qualification (PQ) MUST be version-controlled alongside
             the test suite. Each OQ/PQ execution report MUST reference the git SHA of
             the test data set used, enabling reproducibility and traceability. This
             ensures that test evidence satisfies the ALCOA+ "Original" and "Enduring"
             principles — any future re-execution uses the identical test data, and the
             provenance of test results is traceable to a specific, immutable data
             version.
             Reference: GAMP 5 Appendix D4, ALCOA+ Original, ALCOA+ Enduring.

RECOMMENDED: In non-GxP environments, test data sets used for OQ and PQ SHOULD be
             version-controlled alongside the test suite. Each OQ/PQ execution report
             SHOULD reference the git SHA of the test data set used.
...
## BEH-GD-061: Security Test Plan (§56)
...
| --- | ----------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| 1   | Concurrent evaluation + subject mutation: start evaluation, mutate subject mid-evaluation | Subject is frozen at scope creation ([ADR #9](../decisions/009-immutable-subject-within-scope.md)); mutation throws TypeError in strict mode; evaluation uses original subject         |
...
RECOMMENDED: Organizations deploying @hex-di/guard in GxP environments SHOULD
             ...
             periodic review report (../compliance/gxp.md section 64).
...
| --------------------------- | ------------- | ---------- |
| Race Conditions (§56.1)     | 3             | 3          |
| Replay Attacks (§56.2)      | 3             | 3          |
| DI Manipulation (§56.3)     | 3             | 3          |
| Input Validation (§56.4)    | 3             | 3          |
| Penetration Testing (§56.5) | Guidance only | 0 (manual) |
| **Total**                   | **12**        | **12**     |
...
## BEH-GD-062: Cucumber BDD Acceptance Tests (§57)

Cucumber/Gherkin acceptance tests provide a regulatory-readable test layer for GxP environments. QA personnel and regulatory inspectors can review `.feature` files without understanding TypeScript. The Cucumber runner operates standalone alongside the existing Vitest suite.

### 53.1 Runner Configuration

```
REQUIREMENT: The Cucumber BDD acceptance test suite MUST use @cucumber/cucumber as the
             standalone test runner with tsx as the TypeScript loader. The runner MUST
             be invocable via `pnpm test:cucumber` from the guard package root. Strict
             mode MUST be enabled (undefined steps fail the suite). JSON and HTML
             report output MUST be generated on every run for archival as OQ/PQ evidence.
             Reference: GAMP 5 Category 5 (testing evidence), 21 CFR 11.10(a).
```
...
`GuardCucumberWorld` wraps the memory adapters from `@hex-di/guard-testing` and provides shared state across step definitions within a single scenario. The World is reset per scenario via `Before`/`After` hooks.

```typescript
interface GuardCucumberWorld {
  /** Memory audit trail, reset per scenario. */
  readonly auditTrail: MemoryAuditTrail;
  /** Memory signature service, reset per scenario. */
  readonly signatureService: MemorySignatureService;
  /** Memory WAL store, reset per scenario. */
  readonly walStore: MemoryWalStore;
  /** Memory policy engine, reset per scenario. */
  readonly policyEngine: MemoryPolicyEngine;
  /** Current test subject, set by Given steps. */
  subject: AuthSubject | undefined;
  /** Last evaluation decision, set by When steps. */
  lastDecision: Decision | undefined;
  /** Last signature capture result, set by When steps. */
  lastSignatureResult: Result<ElectronicSignature, SignatureError> | undefined;
  /** Last re-authentication result, set by When steps. */
  lastReauthResult: Result<ReauthenticationToken, SignatureError> | undefined;
  /** Memory attribute resolver for async evaluation scenarios. */
  readonly attributeResolver: MemoryAttributeResolver;
  /** Memory relationship resolver for ReBAC scenarios. */
  readonly relationshipResolver: MemoryRelationshipResolver;
}
```

```
REQUIREMENT: The GuardCucumberWorld MUST be fully reset before each scenario via a
             Before hook. All memory adapters MUST be freshly constructed (not cleared)
             to prevent state leakage between scenarios. The After hook MUST dispose
             of any created scopes.
             Reference: GAMP 5 Category 5 (test isolation).
```

### 53.3 Step Definition Patterns

Step definitions follow a layered pattern: Given steps establish subjects and configuration, When steps perform evaluations or signature operations, Then steps assert on decisions, audit entries, and hash chains.

```
REQUIREMENT: Step definitions MUST use real policy evaluation via the evaluate()
             function and real memory adapter implementations from @hex-di/guard-testing.
             Mocking the evaluate() function or any adapter in Cucumber step definitions
             is PROHIBITED. Step definitions SHOULD reuse utilities from
             @hex-di/guard-testing (createTestSubject, createMemoryAuditTrail, etc.)
             for adapter construction.
             Reference: GAMP 5 Category 5 (testing integrity).
```

Example step definition patterns:

```typescript
// Given steps — establish subjects and state
Given("a subject {string} with role {string}", function (this: GuardCucumberWorld, id: string, role: string) {
  this.subject = createTestSubject({ id, roles: [createRole({ name: role, permissions: [] })] });
});

Given("a subject {string} with permission {string}:{string}", function (
  this: GuardCucumberWorld, id: string, resource: string, action: string
) {
  const perm = createPermission({ resource, action });
  this.subject = createTestSubject({ id, permissions: [perm] });
});

// When steps — perform evaluation
When("the policy {string} is evaluated", function (this: GuardCucumberWorld, policyDesc: string) {
  const policy = parsePolicyDescription(policyDesc); // maps human-readable to policy AST
  this.lastDecision = testPolicy(policy, { subject: this.subject! });
});

// Then steps — assert outcome
Then("the decision should be {string}", function (this: GuardCucumberWorld, expected: string) {
  assert.strictEqual(this.lastDecision?.kind, expected);
});

Then("the audit trail should contain {int} entries", function (this: GuardCucumberWorld, count: number) {
  assert.strictEqual(this.auditTrail.getEntries().length, count);
});
```

### 53.4 Tag Taxonomy

Feature files and scenarios use tags for filtering, regulatory traceability, and CI selection:

| Tag                  | Purpose                                                        |
| -------------------- | -------------------------------------------------------------- |
| `@gxp`               | GxP-relevant scenario (required for regulatory test evidence)  |
| `@audit-trail`       | Audit trail recording and integrity scenarios                  |
| `@hash-chain`        | Hash chain integrity and verification scenarios                |
| `@e-signature`       | Electronic signature capture, validation, and re-auth          |
| `@rbac`              | Role-based access control and permission evaluation            |
| `@separation-of-duties` | Separation of duties enforcement scenarios                  |
| `@break-glass`       | Emergency bypass / break-glass scenarios                       |
| `@admin`             | Administrative operations and policy change control            |
| `@REQ-GUARD-xxx`     | Links scenario to a specific stable requirement ID             |
| `@OQ-xx`             | Links scenario to a specific OQ checklist item                 |
| `@21CFR11.10(a)`     | Regulatory citation tag (FDA)                                  |
| `@AnnexII-S4.4`      | Regulatory citation tag (EU GMP Annex 11)                      |
| `@GAMP5-Cat5`        | Regulatory citation tag (GAMP 5 Category 5)                    |

```
REQUIREMENT: Every scenario in a @gxp-tagged feature file MUST include at least one
             @REQ-GUARD-xxx tag linking it to a stable requirement ID from the
             traceability matrix (11-traceability-matrix.md §69-REQ). Scenarios
             without requirement traceability MUST NOT be counted as GxP test evidence.
             Reference: WHO TRS 996 Annex 5 (bi-directional traceability).
```

### 53.5 Relationship to Existing Tests

The Cucumber BDD layer is complementary to the existing Vitest test suite. It does not replace unit tests, type tests, or conformance suites.

| Test Layer              | Runner  | Purpose                                           | Audience                        |
| ----------------------- | ------- | ------------------------------------------------- | ------------------------------- |
| Unit tests              | Vitest  | Verify individual functions and types              | Developers                      |
| Type tests              | Vitest  | Verify compile-time type constraints               | Developers                      |
| Integration tests       | Vitest  | Verify cross-module interactions                   | Developers, CI                  |
| Conformance suites      | Vitest  | Verify adapter behavioral contracts                | Adapter authors, OQ             |
| **Cucumber BDD**        | **Cucumber** | **Verify regulatory acceptance criteria**     | **QA, inspectors, auditors**    |

```
REQUIREMENT: The Cucumber BDD test suite MUST NOT duplicate the assertions of existing
             Vitest unit and integration tests. BDD scenarios MUST focus on
             regulatory-readable acceptance criteria expressed in Gherkin, not on
             implementation details. Where a BDD scenario exercises the same code path
             as a Vitest test, the BDD scenario provides independent acceptance-level
             evidence — not redundant coverage.
             Reference: GAMP 5 Category 5 (testing strategy).
```

### 53.6 CI Integration

```
REQUIREMENT: The Cucumber BDD test suite MUST be executed in CI on every commit
             alongside the Vitest test suite. Cucumber failures MUST block merge.
             The JSON report output (reports/cucumber-report.json) MUST be archived
             as a CI artifact for OQ evidence retrieval. The HTML report MUST also
             be archived for human-readable inspection.
             Reference: GAMP 5 Category 5 (continuous validation), 21 CFR 11.10(a).
```

### 53.7 Feature File Organization

The BDD acceptance test suite comprises 23 feature files organized across 9 directories, totaling approximately 118 scenarios:

| Directory              | Feature File                       | Scenarios | Tags                                      |
| ---------------------- | ---------------------------------- | --------- | ----------------------------------------- |
| `features/rbac/`       | permission-evaluation.feature      | 8         | @rbac @gxp @REQ-GUARD-037                 |
| `features/rbac/`       | role-hierarchy.feature             | 6         | @rbac @gxp @REQ-GUARD-037                 |
| `features/rbac/`       | separation-of-duties.feature       | 6         | @rbac @separation-of-duties @gxp          |
| `features/audit-trail/`| recording.feature                  | 8         | @audit-trail @gxp @REQ-GUARD-005          |
| `features/audit-trail/`| hash-chain-integrity.feature       | 8         | @audit-trail @hash-chain @gxp             |
| `features/audit-trail/`| completeness.feature               | 4         | @audit-trail @gxp @REQ-GUARD-008          |
| `features/audit-trail/`| field-completeness.feature         | 4         | @audit-trail @gxp @REQ-GUARD-005          |
| `features/e-signature/`| capture-and-validate.feature       | 8         | @e-signature @gxp @REQ-GUARD-028          |
| `features/e-signature/`| re-authentication.feature          | 6         | @e-signature @gxp @REQ-GUARD-029          |
| `features/e-signature/`| counter-signing.feature            | 5         | @e-signature @separation-of-duties @gxp   |
| `features/admin/`      | emergency-bypass.feature           | 6         | @admin @break-glass @gxp                  |
| `features/admin/`      | policy-change-control.feature      | 5         | @admin @gxp @REQ-GUARD-020               |
| `features/gxp/`        | noop-audit-trail-rejection.feature | 3         | @gxp @REQ-GUARD-006                       |
| `features/gxp/`        | scope-disposal-chain-verification.feature | 4   | @gxp @hash-chain @REQ-GUARD-007           |
| `features/gxp/`        | wal-crash-recovery.feature         | 4         | @gxp @REQ-GUARD-006                       |
| `features/rebac/`      | relationship-evaluation.feature    | 6         | @rebac @gxp                                |
| `features/rebac/`      | transitive-relationships.feature   | 4         | @rebac @gxp                                |
| `features/rebac/`      | relationship-field-access.feature  | 4         | @rebac @field-access @gxp                  |
| `features/async/`      | async-attribute-resolution.feature | 5         | @async @gxp                                |
| `features/async/`      | async-timeout-handling.feature     | 3         | @async @gxp                                |
| `features/field-union/`| field-strategy-union.feature       | 4         | @field-union @gxp                          |
| `features/field-union/`| field-strategy-intersection.feature| 3         | @field-union @gxp                          |
| `features/combined/`   | rebac-async-field-union.feature    | 4         | @rebac @async @field-union @gxp            |
| **Total**              |                                    | **118**   |                                           |

```
RECOMMENDED: Each Cucumber feature file SHOULD contain at least 3 scenarios covering
             the positive path, negative path (deny/error), and a boundary condition
             for the feature under test. Feature files with fewer than 3 scenarios
             SHOULD include a documented justification in the feature file header
             comment explaining why fewer scenarios are sufficient (e.g., the feature
             is a single-condition check with no meaningful boundary). This minimum
             ensures adequate scenario diversity for OQ-43 evidence.
             Reference: GAMP 5 Category 5 (test case adequacy).
```

#### Example Feature: Permission Evaluation

```gherkin
@rbac @gxp @REQ-GUARD-037 @21CFR11.10(d)
Feature: Permission-based access control evaluation
  As a GxP compliance officer
  I want guard to enforce permission-based access control
  So that only authorized subjects can access protected resources

  Background:
    Given the guard system is configured with GxP mode enabled

  @OQ-1
  Scenario: Subject with required permission is granted access
    Given a subject "operator-1" with permission "batch:execute"
    When the policy "hasPermission(batch:execute)" is evaluated
    Then the decision should be "allow"
    And the audit trail should contain 1 entry
    And the audit entry should have decision "allow"

  @OQ-1
  Scenario: Subject without required permission is denied access
    Given a subject "viewer-1" with permission "batch:read"
    When the policy "hasPermission(batch:execute)" is evaluated
    Then the decision should be "deny"
    And the denial reason should contain "batch:execute"
    And the audit trail should contain 1 entry
    And the audit entry should have decision "deny"

  Scenario: Subject with inherited permission through role hierarchy
    Given a subject "admin-1" with role "admin" inheriting from "editor" and "viewer"
    And role "viewer" has permission "batch:read"
    And role "editor" has permission "batch:write"
    And role "admin" has permission "batch:release"
    When the policy "hasPermission(batch:read)" is evaluated
    Then the decision should be "allow"
```

#### Example Feature: Hash Chain Integrity

```gherkin
@audit-trail @hash-chain @gxp @REQ-GUARD-010 @21CFR11.10(e)
Feature: Audit trail hash chain integrity
  As a regulatory inspector
  I want every audit entry to be cryptographically chained
  So that tampering with historical records is detectable

  @OQ-6
  Scenario: Genesis entry has valid integrity hash
    Given the guard system is configured with GxP mode enabled
    And an empty audit trail
    When a guard evaluation produces an "allow" decision
    Then the first audit entry should have previousHash equal to ""
    And the first audit entry should have a non-empty integrityHash
    And the hash chain should validate successfully

  @OQ-6
  Scenario: Subsequent entries chain to predecessor
    Given the guard system is configured with GxP mode enabled
    And an audit trail with 3 existing entries
    When a guard evaluation produces a "deny" decision
    Then the new entry's previousHash should equal the predecessor's integrityHash
    And the hash chain should validate successfully for all 4 entries

  @OQ-6
  Scenario: Tampered entry breaks chain verification
    Given the guard system is configured with GxP mode enabled
    And an audit trail with 5 valid chained entries
    When entry 3 is tampered with
    Then the hash chain verification should fail
    And the failure should identify the tampered entry position
```

#### Example Feature: Electronic Signature Capture

```gherkin
@e-signature @gxp @REQ-GUARD-028 @REQ-GUARD-029 @21CFR11.50
Feature: Electronic signature capture and validation
  As a batch release reviewer
  I want to apply a legally binding electronic signature
  So that my approval is cryptographically recorded and verifiable

  @OQ-8
  Scenario: Successful signature capture with re-authentication
    Given a subject "reviewer-1" with role "qa-reviewer"
    And the signature service is configured
    When "reviewer-1" re-authenticates with valid credentials
    And "reviewer-1" captures a signature with meaning "reviewed"
    Then the signature should be captured successfully
    And the signature should have signerId "reviewer-1"
    And the signature should have meaning "reviewed"
    And the signature should have reauthenticated true

  @OQ-8
  Scenario: Signature capture rejected without re-authentication
    Given a subject "reviewer-1" with role "qa-reviewer"
    And the signature service is configured
    When "reviewer-1" attempts to capture a signature without re-authentication
    Then the signature capture should fail with category "reauth_required"

  @OQ-8
  Scenario: Expired re-authentication token is rejected
    Given a subject "reviewer-1" with role "qa-reviewer"
    And the signature service is configured with token validity of 1 millisecond
    When "reviewer-1" re-authenticates with valid credentials
    And 10 milliseconds elapse
    And "reviewer-1" captures a signature with meaning "approved"
    Then the signature capture should fail with category "reauth_expired"
```

#### Example Feature: Emergency Bypass

```gherkin
@admin @break-glass @gxp @21CFR11.10(g)
Feature: Emergency bypass (break-glass) procedures
  As a system administrator
  I want to activate emergency bypass in critical situations
  So that essential operations can proceed while maintaining an audit trail

  Scenario: Emergency bypass activated by authorized admin
    Given the guard system is configured with GxP mode enabled
    And emergency bypass is enabled in AdminGuardConfig
    And a subject "admin-1" with role "guard_admin"
    When "admin-1" activates emergency bypass
    Then the bypass should be activated successfully
    And the audit trail should contain an emergency bypass entry
    And the emergency bypass entry should have an emergency indicator

  Scenario: Emergency bypass rejected when disabled
    Given the guard system is configured with GxP mode enabled
    And emergency bypass is disabled in AdminGuardConfig
    And a subject "admin-1" with role "guard_admin"
    When "admin-1" attempts to activate emergency bypass
    Then the bypass activation should be rejected

  Scenario: Emergency bypass rejected for non-admin subject
    Given the guard system is configured with GxP mode enabled
    And emergency bypass is enabled in AdminGuardConfig
    And a subject "viewer-1" with role "viewer"
    When "viewer-1" attempts to activate emergency bypass
    Then the bypass activation should be rejected with ACL017
```

#### Example Feature: Separation of Duties

```gherkin
@rbac @separation-of-duties @gxp @REQ-GUARD-036 @21CFR11.10(g)
Feature: Separation of duties enforcement
  As a compliance officer
  I want the system to prevent the same person from both proposing and approving
  So that critical operations require independent verification

  @OQ-10
  Scenario: Same signer rejected for counter-signing
    Given the signature service is configured with separation of duties enabled
    And a subject "reviewer-1" with role "qa-reviewer"
    When "reviewer-1" captures a signature with meaning "reviewed" for evaluation "eval-001"
    And "reviewer-1" attempts a second signature with meaning "approved" for evaluation "eval-001"
    Then the second signature should fail with category "separation_violation"

  @OQ-10
  Scenario: Different signers allowed for counter-signing
    Given the signature service is configured with separation of duties enabled
    And a subject "reviewer-1" with role "qa-reviewer"
    And a subject "approver-1" with role "qa-approver"
    When "reviewer-1" captures a signature with meaning "reviewed" for evaluation "eval-001"
    And "approver-1" captures a signature with meaning "approved" for evaluation "eval-001"
    Then both signatures should be captured successfully
```

#### Example Feature: Relationship-Based Access Control

```gherkin
@rebac @gxp
Feature: Relationship-based access control evaluation
  As a document owner
  I want guard to check my relationship to a resource
  So that access is granted based on who owns or views a document

  Background:
    Given the guard system is configured with a relationship resolver
    And a resource "doc-42" of type "document"

  Scenario: Subject with direct owner relationship is granted access
    Given a subject "user-1" with relationship "owner" to resource "doc-42"
    When the policy "hasRelationship(owner)" is evaluated
    Then the decision should be "allow"
    And the audit trail should contain 1 entry

  Scenario: Subject without relationship is denied access
    Given a subject "user-2" with no relationship to resource "doc-42"
    When the policy "hasRelationship(owner)" is evaluated
    Then the decision should be "deny"
    And the denial reason should contain "owner"

  Scenario: Missing resource id produces ACL030
    Given a subject "user-1" with relationship "owner" to resource "doc-42"
    And a resource with no id
    When the policy "hasRelationship(owner)" is evaluated
    Then the evaluation should fail with error code "ACL030"

  Scenario: Missing relationship resolver produces ACL028
    Given the guard system is configured without a relationship resolver
    And a subject "user-1"
    When the policy "hasRelationship(owner)" is evaluated
    Then the evaluation should fail with error code "ACL028"

  Scenario: Transitive relationship with depth 2
    Given a subject "user-1" with relationship "member" to group "team-a"
    And group "team-a" with relationship "member" to org "org-1"
    And a resource "doc-42" owned by org "org-1"
    When the policy "hasRelationship(member, depth: 2)" is evaluated
    Then the decision should be "allow"

  Scenario: Transitive relationship exceeding depth bound is denied
    Given a subject "user-1" with relationship chain of depth 3
    When the policy "hasRelationship(member, depth: 2)" is evaluated
    Then the decision should be "deny"
```

#### Example Feature: Async Attribute Resolution

```gherkin
@async @gxp
Feature: Async attribute resolution for ABAC policies
  As a system administrator
  I want guard to resolve missing subject attributes on demand
  So that I do not need to pre-fetch all attributes for every evaluation

  Background:
    Given the guard system is configured with an attribute resolver
    And the resolver provides attribute "clearance" with value "top-secret"

  Scenario: Missing attribute resolved on demand
    Given a subject "analyst-1" without attribute "clearance"
    When the policy "hasAttribute(clearance, eq(top-secret))" is evaluated asynchronously
    Then the decision should be "allow"
    And the trace should indicate async resolution was used
    And the trace should include resolution duration

  Scenario: Pre-populated attribute skips resolution
    Given a subject "analyst-1" with attribute "clearance" set to "top-secret"
    When the policy "hasAttribute(clearance, eq(top-secret))" is evaluated asynchronously
    Then the decision should be "allow"
    And the trace should not indicate async resolution

  Scenario: Attribute resolution timeout produces ACL026
    Given an attribute resolver with a 100ms timeout
    And the resolver takes 200ms to respond
    And a subject "analyst-1" without attribute "clearance"
    When the policy "hasAttribute(clearance, eq(top-secret))" is evaluated asynchronously
    Then the evaluation should fail with error code "ACL026"

  Scenario: Attribute resolution error produces ACL027
    Given an attribute resolver that throws an error
    And a subject "analyst-1" without attribute "clearance"
    When the policy "hasAttribute(clearance, eq(top-secret))" is evaluated asynchronously
    Then the evaluation should fail with error code "ACL027"

  Scenario: Resolved attributes cached within single evaluation
    Given a subject "analyst-1" without attribute "clearance"
    And a policy requiring "clearance" in two sub-policies
    When the combined policy is evaluated asynchronously
    Then the attribute resolver should have been called exactly once for "clearance"
```

#### Example Feature: Field Union Strategy

```gherkin
@field-union @gxp
Feature: Field-level union strategy for composite policies
  As a data steward
  I want to configure how field grants are merged across policies
  So that subjects can see the union of fields they are authorized for

  Scenario: anyOf with union merges fields from all allowing children
    Given a subject "user-1" matching both "viewer" and "editor" policies
    And "viewer" grants fields "title" and "summary"
    And "editor" grants fields "title" and "content"
    When the policy "anyOf(viewer, editor, fieldStrategy: union)" is evaluated
    Then the decision should be "allow"
    And the visible fields should be "title", "summary", "content"

  Scenario: anyOf with union evaluates all children (no short-circuit)
    Given a subject "user-1" matching the first child policy
    When the policy "anyOf(child-a, child-b, fieldStrategy: union)" is evaluated
    Then all child policies should have been evaluated
    And the trace should have complete flag true

  Scenario: allOf with default intersection restricts fields
    Given a subject "user-1" matching all child policies
    And "policy-a" grants fields "title", "content", "metadata"
    And "policy-b" grants fields "title", "content"
    When the policy "allOf(policy-a, policy-b)" is evaluated
    Then the visible fields should be "title", "content"

  Scenario: allOf with union expands fields
    Given a subject "user-1" matching all child policies
    And "policy-a" grants fields "title", "content", "metadata"
    And "policy-b" grants fields "title", "summary"
    When the policy "allOf(policy-a, policy-b, fieldStrategy: union)" is evaluated
    Then the visible fields should be "title", "content", "metadata", "summary"
```

#### Example Feature: Cross-Enhancement Composition

```gherkin
@rebac @async @field-union @gxp
Feature: Combined ReBAC, async evaluation, and field union
  As a compliance officer
  I want all three authorization models to compose in a single policy
  So that complex access control scenarios are handled correctly

  Scenario: Owner gets all fields, viewer gets subset via union
    Given a subject "user-1" with relationships "owner" and "viewer" to resource "doc-1"
    And "owner" grants fields "title", "content", "metadata"
    And "viewer" grants fields "title", "summary"
    When the policy combining hasRelationship with anyOf union is evaluated
    Then the decision should be "allow"
    And the visible fields should be "title", "content", "metadata", "summary"

  Scenario: Async attribute resolution combined with ReBAC
    Given a subject "user-1" with relationship "viewer" to resource "doc-1"
    And the subject is missing attribute "classification"
    And the resolver provides "classification" with value "internal"
    When the combined ReBAC + ABAC policy is evaluated asynchronously
    Then the decision should be "allow"
    And the trace should indicate both relationship check and async resolution

  Scenario: Denied relationship with async attribute is still denied
    Given a subject "user-1" with no relationship to resource "doc-1"
    And the resolver provides "classification" with value "internal"
    When the combined ReBAC + ABAC policy is evaluated asynchronously
    Then the decision should be "deny"
    And the attribute resolver should not have been called

  Scenario: Full composition audit trail completeness
    Given a subject "user-1" with relationship "owner" to resource "doc-1"
    And the resolver provides "classification" with value "public"
    When the combined ReBAC + ABAC + field union policy is evaluated asynchronously
    Then the audit trail should contain 1 entry
    And the audit entry should include the full evaluation trace
    And the trace should show relationship check, async resolution, and field strategy
```
...
| -------------------------------- | ------------- | --------- |
| RBAC (§57.7 — `features/rbac/`) | 3             | 20        |
| Audit Trail (`features/audit-trail/`) | 4         | 24        |
| Electronic Signatures (`features/e-signature/`) | 3 | 19        |
| Administrative (`features/admin/`) | 2            | 11        |
| GxP-Specific (`features/gxp/`)  | 3             | 11        |
| ReBAC (`features/rebac/`)        | 3             | 14        |
| Async Evaluation (`features/async/`) | 2          | 8         |
| Field Union (`features/field-union/`) | 2          | 7         |
| Cross-Enhancement (`features/combined/`) | 1       | 4         |
| **Total**                        | **23**        | **118**   |

> **Note:** The 118 Cucumber scenarios are in addition to the 1176 existing Vitest tests. The Cucumber layer focuses on regulatory-readable acceptance criteria, while the Vitest suite covers unit tests, type tests, integration tests, and conformance suites. Together they provide 1294 tests across all layers.

---

_Previous: [12 - Inspection](./11-inspection.md) | Next: [14 - API Reference](../14-api-reference.md)_
