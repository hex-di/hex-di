...
> | Change History   | 1.2 (2026-02-15): Added explicit Threat Model Limitation statement to GxP Compliance Warning section documenting client-side bypassability (CCR-GUARD-013) |
> |                  | 1.1 (2026-02-19): Added BEH-GD-NNN requirement identifiers to section headings (CCR-GUARD-020) |
> |                  | 1.1 (2026-02-14): Added §46 GxP Suitability section with subject freshness, fail-closed error boundaries, and scope lifetime requirements (CCR-GUARD-006) |
> |                  | 1.0 (2026-02-13): Initial controlled release |

_Previous: [10 - Guard Integration Contracts](./09-cross-library.md)_

---

## BEH-GD-042: SubjectProvider (§41)

> **Invariant:** [INV-GD-004](../invariants.md) — Subject Immutability Within Scope (SubjectProvider manages scope)
> **DoD:** [DoD 9: React SubjectProvider](../16-definition-of-done.md#dod-9-react-subjectprovider), [DoD 10: React Can/Cannot](../16-definition-of-done.md#dod-10-react-cancannot), [DoD 11: React Hooks](../16-definition-of-done.md#dod-11-react-hooks)

A pure React context provider that makes the authorization subject available to descendant components (see [ADR-GD-005](../decisions/005-react-subject-provider-context.md)). Following architecture review decision #5, this is NOT a DI scope -- it is a React context provider only. The subject flows through React context, not through the DI container.
...
## BEH-GD-043: Can and Cannot Components (§42)
...
## BEH-GD-044: useCan and useCanDeferred Hooks (§43)
...
## BEH-GD-045: usePolicy and usePolicyDeferred Hooks (§44)
 ...
 * stable, and subjects change infrequently.
 *
 * @param policy   - The policy to evaluate
 * @param options  - Optional: resource context for resource-based policies
 * @returns Decision
 *
 * @throws MissingSubjectProviderError if called outside SubjectProvider tree
 */
...
function usePolicyDeferred(
  ...
  return useMemo(
    () => ({
      status: "resolved",
      decision: evaluate(policy, { subject: precomputed, resource: options?.resource }),
    }),
    [policy, options?.resource, precomputed]
  );
}
```
...
Both `usePolicy` and `usePolicyDeferred` wrap evaluation in `useMemo` keyed by `[policy, resource, subject]`. Since policies are frozen (stable reference) and subjects change infrequently (once per login/scope), this memoizes effectively.

---

## BEH-GD-046: useSubject and useSubjectDeferred Hooks (§45)
...
Following the `createTypedHooks()` factory pattern from `integrations/react/src/factories/create-typed-hooks.tsx`, `createGuardHooks()` creates an isolated set of guard hooks and components bound to a specific type parameter (see [ADR #15](../decisions/015-create-guard-hooks-factory.md)). This enables applications with multiple guard contexts (e.g., main app + embedded widget with different subjects).
...
## BEH-GD-047: usePolicies and usePoliciesDeferred Hooks (§73)
...
>
> **GxP-regulated operations MUST go through the server-side `guard()` wrapper**, which evaluates policies, records audit entries, and enforces the full compliance contract defined in 07-guard-adapter.md and ../compliance/gxp.md.
>
...
>
> **Threat Model Limitation:** Client-side authorization is inherently bypassable because the browser runtime is untrusted. An attacker with browser developer tools, a modified client, or a direct API client can bypass all React gate checks. The STRIDE threat model (§68a) classifies React client-side threats (S/T/R/E/D-React-1) as Low or Moderate severity specifically because server-side `guard()` enforcement is the primary control. Organizations MUST NOT rely on React gates as compensating controls for server-side authorization gaps.
...
For ReBAC policies in React, a `RelationshipResolverProvider` supplies the relationship resolver to the component tree. This is separate from `SubjectProvider` because the resolver is an infrastructure concern, not a subject concern.

```typescript
/**
 * Provides a RelationshipResolver to the component tree for ReBAC evaluation.
 *
 * Must wrap components that use useCan/usePolicy with hasRelationship policies.
 * When not provided and a hasRelationship policy is evaluated, the evaluation
 * returns Err(MissingRelationshipResolver).
 */
interface RelationshipResolverProviderProps {
  readonly resolver: RelationshipResolver;
  readonly children: ReactNode;
}

function RelationshipResolverProvider(props: RelationshipResolverProviderProps): ReactElement;
```

### Usage

```typescript
import { SubjectProvider, RelationshipResolverProvider, Can } from "@hex-di/guard/react";

function App({ user, resolver }: { user: AuthSubject; resolver: RelationshipResolver }) {
  return (
    <SubjectProvider subject={user}>
      <RelationshipResolverProvider resolver={resolver}>
        <Can policy={hasRelationship("owner")}>
          <EditButton />
        </Can>
      </RelationshipResolverProvider>
    </SubjectProvider>
...
For async evaluation (attribute resolution, async relationship checks), `useCanAsync` wraps `evaluateAsync()` and integrates with React Suspense.

```typescript
/**
 * Async variant of useCan that supports on-demand attribute resolution.
 *
 * Suspends (via React Suspense) while async resolution is in progress.
 * Returns the same CanResult as useCan once resolution completes.
 *
 * Note: React Suspense already handles the async boundary. This hook
 * internally calls evaluateAsync() and wraps the Promise for Suspense.
 *
 * @param policy - The policy to evaluate
 * @param resource - Optional resource context
 * @returns CanResult (same as useCan)
 */
function useCanAsync(
  policy: PolicyConstraint,
  resource?: Resource
): CanResult;
```
...
```
RECOMMENDED: Applications using useCanAsync SHOULD wrap the component tree in
             a <Suspense> boundary with an appropriate loading fallback.
             The Suspense boundary should be above the component using useCanAsync
             but below SubjectProvider.
```

---

## BEH-GD-048: GxP Suitability (§46)

> **Invariants:**
> - [INV-GD-020](../invariants.md) — Scope Lifetime Enforcement
> - [INV-GD-021](../invariants.md) — Subject Authentication Staleness

React integration components run in the browser — an untrusted environment where the user controls the runtime. All guard React hooks and components provide **UI convenience** (showing/hiding elements), not **security enforcement**. Server-side authorization via `guard()` remains the authoritative enforcement mechanism.

However, in GxP environments, stale or incorrect client-side authorization state can lead to misleading UI (e.g., showing a "Release Batch" button to a user whose access has been revoked), which undermines the principle of least privilege and may confuse audit reviewers.

### Subject Freshness

```
REQUIREMENT: When gxp is true in the guard configuration, React applications
             MUST implement subject freshness validation. SubjectProvider MUST
             re-fetch or re-validate the subject at an interval no greater than
             the maxScopeLifetimeMs configured on the guard graph (see
             06-administrative-controls.md §64g-4). When the refresh interval
             elapses, the subject MUST be set to null (triggering Suspense) until
             the refreshed subject is available.

             Implementation approaches (choose one):
             (a) Polling: Use setInterval at maxScopeLifetimeMs to re-fetch the
                 subject from the identity provider.
             (b) Token expiry: If the subject is derived from a JWT or session
                 token, monitor the token's exp claim and set the subject to null
                 when expired.
             (c) Server push: Use a WebSocket or SSE channel to receive subject
                 invalidation events from the server.
...
```
RECOMMENDED: SubjectProvider SHOULD accept an optional refreshIntervalMs prop
             for GxP environments. When set, SubjectProvider schedules automatic
             subject re-validation at the specified interval. If the application
             does not provide this prop and gxp is true, the guard DevTools
             inspector SHOULD emit a warning diagnostic
             ("guard.react-no-refresh-interval") on first render.
```

### Fail-Closed Error Boundaries

```
REQUIREMENT: When gxp is true, React applications using guard hooks MUST wrap
             guarded component trees in a fail-closed error boundary. If a guard
             hook throws an unexpected error (not a MissingSubjectProviderError,
             which is a configuration error), the error boundary MUST:

             (a) Render a denial fallback UI (not the protected content).
             (b) Log the error via the application's logging adapter with
                 sufficient context for incident investigation (error message,
                 component stack, subjectId if available).
             (c) NOT retry the failed authorization check automatically.
                 Manual user action (e.g., page refresh, re-login) MUST be
                 required to recover.

             This prevents a transient error from granting access to protected
             UI elements.
             Reference: 21 CFR 11.10(d), EU GMP Annex 11 §13
             (incident management).
```

```typescript
/**
 * Example fail-closed error boundary for GxP React applications.
 *
 * Catches errors from guard hooks and renders a denial fallback
 * instead of the protected content.
 */
interface GuardErrorBoundaryProps {
  /** Rendered when a guard hook throws an unexpected error. */
  readonly fallback: ReactNode;
  /** Optional callback for logging/alerting on guard errors. */
  readonly onError?: (error: Error) => void;
  /** Protected content. */
  readonly children: ReactNode;
}

function GuardErrorBoundary(props: GuardErrorBoundaryProps): ReactElement;
```

### Stale Subject Detection

```
REQUIREMENT: When gxp is true, useCan, usePolicy, and useCanDeferred MUST
             compare the current subject's authenticatedAt timestamp against
             the current time (via ClockSource or Date.now()). If the elapsed
             time exceeds maxScopeLifetimeMs, the hooks MUST treat the subject
             as stale and:
             (a) Suspense hooks (useCan, usePolicy): suspend, forcing the
                 application to re-fetch the subject.
             (b) Deferred hooks (useCanDeferred, usePolicyDeferred): return
                 { status: "pending" }, indicating the subject requires refresh.
             This prevents authorization decisions based on a subject whose
             session may have been revoked or whose roles may have changed.
             Reference: 21 CFR 11.10(d), EU GMP Annex 11 §12.4.
```

### Client-Side Authorization as UI Convenience Only

```
REQUIREMENT: All @hex-di/guard/react documentation, JSDoc comments, and
             README content MUST include a prominent notice that client-side
             authorization via React hooks and components is a UI convenience
             only and does NOT replace server-side enforcement via guard().
             In GxP environments, the authoritative authorization decision
             is always the server-side guard() evaluation recorded in the
             audit trail. Client-side checks are supplementary and MUST NOT
             be relied upon as the sole authorization mechanism for any
             GxP-regulated operation.
             Reference: 21 CFR 11.10(d), OWASP ASVS V4 (Access Control).
```

---

_Previous: [10 - Guard Integration Contracts](./09-cross-library.md) | Next: [12 - Inspection](./11-inspection.md)_
