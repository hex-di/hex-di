...
| Change History   | 1.1 (2026-02-21): Groups 29/30/31 spec refs §72→§84, §73→§85, §74→§86 (section renumbering to resolve collisions) (CCR-GUARD-045) |
...
- Acceptance: Snapshot tests for all 10 policy kinds
...
- Acceptance: Round-trip test for all 10 kinds, error cases for invalid input
...
**Roadmap:** Item 9 | **Spec:** Section 41 | **Dependencies:** Task Group 6
...
**Roadmap:** Item 10 | **Spec:** Sections 42-45 | **Dependencies:** Task Group 9
...
**Roadmap:** Item 11 | **Spec:** Sections 43-45 | **Dependencies:** Task Groups 9, 10
...
- Returns `{ SubjectProvider, Can, Cannot, useCan, usePolicy, useSubject, useCanDeferred, usePolicyDeferred, useSubjectDeferred, usePolicies, usePoliciesDeferred }` (11 members)
...
- Export individual components and all 8 hooks as named exports
- Export `CanResult`, `PolicyResult`, `PoliciesDecisions`, `PoliciesResult` types
- Acceptance: `import { SubjectProvider, useCan, usePolicies, CanResult, PolicyResult } from "@hex-di/guard/react"` works
...
- Test usePolicies: suspends on null, PoliciesDecisions<M> when loaded
- Test usePoliciesDeferred: pending/resolved states with PoliciesResult<M>
- Test createGuardHooks: isolation between instances (11 members)
...
**Roadmap:** Item 12 | **Spec:** Sections 47-48 | **Dependencies:** Task Groups 5, 7
...
**Roadmap:** Cross-cutting | **Spec:** 01-overview.md section 5, 12-inspection.md sections 48c-48d | **Dependencies:** Task Group 12
...
- Acceptance: Exhaustive switch compiles with all 10 kinds, combinator returns correct shape
...
- Add ring buffer WARNING in `12-inspection.md` section 47
...
## Task Group 24: Array Matchers (`someMatch` and `contains`)

**Roadmap:** Ergonomic Improvements | **Spec:** §16 extension, §18 extension, §31 extension | **Dependencies:** Groups 5 (Policy Evaluator), 8 (Serialization)

### 24.1 Add `someMatch` and `contains` to `MatcherKind` and `MatcherExpression`

- Extend `MatcherKind` union with `"someMatch"` and `"contains"`
- Define `ObjectMatcher` type: `Readonly<Record<string, MatcherExpression>>`
- Add `{ kind: "someMatch", matcher: ObjectMatcher }` variant to `MatcherExpression`
- Add `{ kind: "contains", ref: MatcherReference }` variant to `MatcherExpression`
- Acceptance: Type tests verify discriminated union extension; no existing match exhaustiveness broken

### 24.2 Implement `someMatch` and `contains` builder functions

- `someMatch(matcher: ObjectMatcher): MatcherExpression` — returns frozen object
- `contains(ref: MatcherReference): MatcherExpression` — returns frozen object
- Export from `@hex-di/guard` and `@hex-di/guard/combinators`
- Acceptance: Unit tests confirm frozen output, correct `kind` discriminant

### 24.3 Add `someMatch` and `contains` evaluation logic in `evaluateMatcher()`

- `someMatch`: missing attribute → false (deny); non-array → `PolicyEvaluationError(ACL003)`; uses `Array.prototype.some` with recursive `evaluateMatcher` per ObjectMatcher field
- `contains`: non-array attribute → `PolicyEvaluationError(ACL003)`; uses `Array.prototype.includes` (strict equality)
- `resource()` references inside ObjectMatcher resolve against `EvaluationContext.resource`
- Acceptance: Unit tests for all evaluation paths (DoD 25 checklist)

### 24.4 Add serialization and deserialization for new matchers

- `serializePolicy` handles `kind: "someMatch"` (serializes nested `ObjectMatcher` as plain object)
- `serializePolicy` handles `kind: "contains"` (serializes `ref` field)
- `deserializePolicy` validates and reconstructs both new matcher variants
- Acceptance: Round-trip tests; deserialize of unknown kind returns `PolicyParseError(ACL007)`

### 24.5 Write unit and type tests (DoD 25)

- ~20 unit tests covering all evaluation paths (allow, deny, error, edge cases)
- ~10 type tests for `ObjectMatcher` assignability, `MatcherExpression` union, builder return types
- ~5 integration tests: end-to-end policy with `someMatch` + `resource()`, serialization round-trip
- Acceptance: All DoD 25 verification items pass; `pnpm test` and `pnpm test:types` green

---

## Task Group 25: Subject Enrichment Utilities (`withAttributes`, `getAttribute`)

**Roadmap:** Ergonomic Improvements | **Spec:** §72 | **Dependencies:** Group 6 (Subject Port)

### 25.1 Implement `withAttributes()` — immutable attribute merge

- `withAttributes(subject, attributes)` returns new object without mutating input
- New attributes are merged over existing (new keys take precedence on conflict)
- If subject is `PrecomputedSubject`, preserves `roleSet` and `permissionSet`
- Acceptance: Unit test confirms original subject unchanged; PrecomputedSubject fields preserved

### 25.2 Implement `getAttribute<T>()` — typed accessor with validator

- `getAttribute(subject, key, validate)` returns `T | undefined`
- Returns `undefined` when attribute absent or validator returns `false`
- Acceptance: Unit tests cover hit, miss, and type-validation failure paths

### 25.3 Add `AuthSubjectAttributes` module augmentation interface

- Export `AuthSubjectAttributes` as empty open interface from `@hex-di/guard`
- Update `AuthSubject.attributes` type to `Readonly<Record<string, unknown> & AuthSubjectAttributes>`
- Document augmentation pattern in JSDoc with `@example`
- Acceptance: Type tests confirm augmentation enables typed attribute field access

### 25.4 Write unit and type tests (DoD 26)

- ~12 unit tests: immutability, merge precedence, PrecomputedSubject preservation, getAttribute paths
- ~8 type tests: withAttributes return type, getAttribute generic inference, AuthSubjectAttributes augmentation
- Acceptance: All DoD 26 verification items pass

---

## Task Group 26: `usePolicies` / `usePoliciesDeferred` Hooks

**Roadmap:** Ergonomic Improvements | **Spec:** §73 | **Dependencies:** Groups 11 (React Hooks), 25 (withAttributes)

### 26.1 Implement `usePolicies` (Suspense variant)

- Suspends when `subject` is `null` (delegates loading to `<Suspense>`)
- Calls `enricher` once with resolved subject before all evaluations (if provided)
- Calls `evaluate(policy, context)` once per key in `policies` map
- Memoizes decisions record by subject + policies + enricher + resource identity (referential stability)
- Acceptance: Unit test confirms suspension behavior and memoization

### 26.2 Implement `usePoliciesDeferred` (non-Suspense variant)

- Returns `{ status: 'pending' }` when subject is `null`
- Returns `{ status: 'ready', decisions: PoliciesDecisions<M> }` when subject is available
- Same enricher and resource semantics as `usePolicies`
- Acceptance: Unit test confirms pending/ready states without suspension

### 26.3 Add `usePolicies` and `usePoliciesDeferred` to `createGuardHooks()` factory

- Both hooks included in `GuardHooks` interface returned by `createGuardHooks()`
- Each `createGuardHooks()` instance has isolated context
- Default exports from `@hex-di/guard/react` delegate to default hooks instance
- Acceptance: `createGuardHooks()` result includes both new hooks; isolation test passes

### 26.4 Write unit and type tests (DoD 27)

- ~15 unit tests: suspend/ready states, enricher called once, resource context, memoization stability
- ~8 type tests: `PoliciesDecisions<M>` generic inference, `PoliciesResult<M>` discriminant, `SubjectEnricher`, `GuardHooks` interface
- ~5 integration tests: ContentCard-style component, `createGuardHooks` isolation, enricher + resource together
- Acceptance: All DoD 27 verification items pass

---

## Task Group 27: `createRoleGate` Factory

**Roadmap:** Ergonomic Improvements | **Spec:** §71 | **Dependencies:** Group 24 (Array Matchers)

### 27.1 Implement `createRoleGate()` — generates `AnyOfPolicy` from bindings

- `createRoleGate(bindings)` returns plain `AnyOfPolicy<readonly PolicyConstraint[]>`
- Binding without `requires` → `hasRole(role)` only
- Binding with `requires` → `allOf(hasRole(role), requires)`
- `requires` field accepts any `PolicyConstraint` (domain-agnostic)
- Export `RoleGateBinding` interface from `@hex-di/guard`
- Acceptance: Generated policy structure compared against manually constructed equivalent; structurally equal

### 27.2 Verify output is fully serializable via `serializePolicy()`

- `serializePolicy(createRoleGate(...))` must succeed for all binding configurations
- `deserializePolicy(serializePolicy(createRoleGate(...)))` must produce structurally equal policy
- No closures or function references in output — pure plain data
- Acceptance: Round-trip serialization tests pass

### 27.3 Write unit and type tests (DoD 28)

- ~10 unit tests: binding without requires, binding with requires, mixed bindings, empty bindings, serialization round-trip, structural equality
- ~6 type tests: `RoleGateBinding` shape, `requires` as arbitrary `PolicyConstraint`, return type `AnyOfPolicy`
- ~3 integration tests: end-to-end evaluation with `hasAttribute + someMatch` requires, serialize + deserialize + evaluate round-trip
- Acceptance: All DoD 28 verification items pass; `createRoleGate` output evaluates correctly with `resource()` context

---

## Task Group 28: v3 Array Matchers (`everyMatch`, `size`)

**Roadmap:** Ergonomic Improvements | **Spec:** §16 v3, §18 v3, §31 v3 | **Dependencies:** Group 24 (Array Matchers)

### 28.1 Add `everyMatch` and `size` to `MatcherKind` and `MatcherExpression`

- Extend `MatcherKind` with `"everyMatch"` and `"size"` variants
- Extend `MatcherExpression` union with `{ kind: "everyMatch", matcher: ObjectMatcher }` and `{ kind: "size", matcher: MatcherExpression }`
- Acceptance: TypeScript exhaustiveness checks compile clean; no `default` case needed

### 28.2 Implement `everyMatch()` and `size()` builder functions

- `everyMatch(matcher: ObjectMatcher): MatcherExpression` — returns frozen expression
- `size(matcher: MatcherExpression): MatcherExpression` — returns frozen expression
- Export both from `@hex-di/guard`
- Acceptance: Builder unit tests pass

### 28.3 Add evaluation logic for `everyMatch` and `size` in `evaluateMatcher()`

- `everyMatch`: missing attribute → deny; non-array → `ACL003`; empty array → deny; every item must satisfy all ObjectMatcher fields → allow
- `size`: non-array → `ACL003`; extracts `array.length`, passes to inner numeric matcher
- Acceptance: All DoD 29 unit tests pass

### 28.4 Add serialization/deserialization for `everyMatch` and `size`

- `serializePolicy` handles both new kinds (same structure as `someMatch` and `size` respectively)
- `deserializePolicy` validates and reconstructs both kinds
- Acceptance: Round-trip tests pass; invalid inputs return `PolicyParseError(ACL007)`

### 28.5 Write unit, type, and integration tests (DoD 29)

- ~18 unit tests, ~8 type tests, ~4 integration tests
- Acceptance: All DoD 29 verification items pass

---

## Task Group 29: `hasResourceAttribute` Policy

**Roadmap:** Ergonomic Improvements | **Spec:** §84 policy type, §18 evaluator, §31 serialization | **Dependencies:** Groups 3 (Policy Data Types), 5 (Evaluator), 8 (Serialization)

### 29.1 Add `HasResourceAttributePolicy` interface and builder

- `HasResourceAttributePolicy<TAttribute>` with `kind: "hasResourceAttribute"`, `attribute`, `matcher`, `fields?`
- `hasResourceAttribute<const A extends string>(attribute, matcher, options?)` builder exported from `@hex-di/guard`
- Add to `Policy` union and `PolicyKind` union
- Add to exhaustive traversal switch in existing examples
- Acceptance: TypeScript exhaustiveness checks compile clean

### 29.2 Implement evaluation for `hasResourceAttribute` in `evaluateNode()`

- Reads from `context.resource?.[policy.attribute]`, NOT `subject.attributes`
- Missing resource or missing field → deny (not error)
- Calls `evaluateMatcher()` with the resource field value
- `fields` option propagated to `Allow.visibleFields`
- Acceptance: Unit tests cover missing resource, missing field, matching, non-matching cases

### 29.3 Add serialization/deserialization for `hasResourceAttribute`

- JSON form: `{ "kind": "hasResourceAttribute", "attribute": "...", "matcher": {...} }`
- Same structure as `hasAttribute` except `kind` discriminant
- Deserializer validates attribute is non-empty string
- Acceptance: Round-trip tests pass

### 29.4 Write unit, type, and integration tests (DoD 30)

- ~12 unit tests, ~6 type tests, ~3 integration tests
- Acceptance: All DoD 30 verification items pass

---

## Task Group 30: `withLabel` Policy Wrapper

**Roadmap:** Ergonomic Improvements | **Spec:** §85 policy type, §18 evaluator, §31 serialization | **Dependencies:** Groups 3 (Policy Data Types), 5 (Evaluator), 8 (Serialization)

### 30.1 Add `LabeledPolicy` interface and `withLabel` builder

- `LabeledPolicy<TPolicy>` with `kind: "labeled"`, `label: string`, `policy: TPolicy`
- `withLabel<P extends PolicyConstraint>(label: string, policy: P): LabeledPolicy<P>` exported from `@hex-di/guard`
- Add to `Policy` union and `PolicyKind` union
- Add to exhaustive traversal switch in existing examples
- Acceptance: TypeScript exhaustiveness checks compile clean

### 30.2 Implement transparent evaluation for `labeled` in `evaluateNode()`

- Evaluates inner policy, replaces `trace.label` with `policy.label`
- All other decision fields (`kind`, `reason`, `visibleFields`, `evaluationId`) preserved from inner result
- `fields` restriction from inner policy propagated unmodified
- Acceptance: Decision outcome identical to evaluating inner policy directly

### 30.3 Add serialization/deserialization for `labeled`

- JSON form: `{ "kind": "labeled", "label": "...", "policy": {...} }`
- Deserializer validates `label` is non-empty string; invalid → `PolicyParseError(ACL007)`
- Acceptance: Round-trip tests pass; nested `withLabel` round-trips correctly

### 30.4 Write unit, type, and integration tests (DoD 31)

- ~10 unit tests, ~5 type tests, ~3 integration tests
- Acceptance: All DoD 31 verification items pass; GxP audit trail output shows label

---

## Task Group 31: `evaluateBatch` Server-Side Batch Evaluation

**Roadmap:** Ergonomic Improvements | **Spec:** §74 | **Dependencies:** Group 5 (Evaluator)

### 31.1 Implement `evaluateBatch()` function

- `evaluateBatch<M extends PoliciesMap>(policies, context, options?)` exported from `@hex-di/guard`
- `PoliciesMap = Readonly<Record<string, PolicyConstraint>>` — same type as used by React hooks
- Returns `{ readonly [K in keyof M]: Result<Decision, PolicyEvaluationError> }`
- Evaluates in insertion order; individual errors captured per key (no abort-on-first-error)
- Acceptance: Error isolation test passes (one invalid policy does not skip others)

### 31.2 Verify structural parity with React `usePolicies`

- Same `PoliciesMap` object usable server-side and in React hooks
- Acceptance: Integration test passes using identical `PoliciesMap` with both APIs

### 31.3 Write unit, type, and integration tests (DoD 32)

- ~8 unit tests, ~4 type tests, ~3 integration tests
- Acceptance: All DoD 32 verification items pass

---

## Task Group 32: `resource` Option on Single-Policy Hooks

**Roadmap:** Ergonomic Improvements | **Spec:** §41 update | **Dependencies:** Group 11 (React Hooks), Group 29 (hasResourceAttribute)

### 32.1 Add `resource` option to `usePolicy` and `usePolicyDeferred`

- `usePolicy(policy, options?: { resource?: Resource })` — forward `resource` to `EvaluationContext`
- `usePolicyDeferred(policy, options?: { resource?: Resource })` — same
- Update `createGuardHooks()` return type `GuardHooks` with updated signatures
- Update memoization key to include `resource` reference
- Acceptance: `hasResourceAttribute` and `resource()` matcher references work in single-policy hooks

### 32.2 Write unit, type, and integration tests (DoD 33)

- ~8 unit tests, ~4 type tests, ~2 integration tests
- Acceptance: All DoD 33 verification items pass; backward compatibility confirmed (no options = no regression)

---

## Task Group 33: `hasRelationship` Policy (ReBAC)

**Roadmap:** ReBAC | **Spec:** §04 (policy type), §05 (evaluator), §06 (§22b, RelationshipResolverPort), §07 (guard auto-detection), §08 (serialization) | **Dependencies:** Groups 3 (Policy Data Types), 4 (Combinators), 5 (Evaluator), 6 (Subject Port), 7 (Guard Adapter), 8 (Serialization) | **DoD:** DoD 27

### 33.1 Add `HasRelationshipPolicy` interface and `hasRelationship` builder

- `HasRelationshipPolicy<TRelation>` with `kind: "hasRelationship"`, `relation: TRelation`, `resourceType?: string`, `depth?: number` (default 1), `fields?: readonly string[]`
- `hasRelationship<const R extends string>(relation: R, options?: HasRelationshipOptions): HasRelationshipPolicy<R>` exported from `@hex-di/guard`
- `RelationshipResolver` interface: `check(subjectId, relation, resourceId, options?): boolean`, `checkAsync(subjectId, relation, resourceId, options?): Promise<boolean>`
- `RelationshipResolverPort` — optional outbound infrastructure port (`port<RelationshipResolver>()({ name: "RelationshipResolver", direction: "outbound", category: "guard/relationship" })`)
- Add `hasRelationship` to `Policy` union, `PolicyKind` union, and exhaustive traversal switch
- Acceptance: TypeScript exhaustiveness checks compile clean; `PolicyKind` union has 10 members

### 33.2 Implement evaluation for `hasRelationship` in `evaluateNode()`

- Sync path: calls `resolver.check(context.subject.id, policy.relation, context.resource?.id, { depth, resourceType })`
- Async path: `evaluateAsync()` calls `resolver.checkAsync(...)`
- `resourceId` from `context.resource?.id` — missing → `Err(ACL030: MissingResourceId)`
- Missing resolver in context → `Err(ACL028: MissingRelationshipResolver)`
- Resolver throws → `Err(ACL029: RelationshipResolutionError)`
- Depth bounds enforced per-policy (default 1); `depth: 2` follows at most 2 hops
- `fields` option propagated to `Allow.visibleFields` on successful evaluation
- Trace label format: `hasRelationship(${relation}, ${resourceType ?? "resource"})`
- Acceptance: Unit tests cover direct, transitive (depth > 1), typed (resourceType), field propagation, all three error cases

### 33.3 Add `RelationshipResolverPort` auto-detection to `guard()`

- `guard()` inspects policy tree for any `hasRelationship` node; if found, adds `RelationshipResolverPort` to `requires`
- `guardAsync()` same detection; resolver port is optional (absent → sync error at evaluation time)
- Acceptance: Integration test: guard adapter automatically receives `RelationshipResolverPort` when policy contains `hasRelationship`

### 33.4 Add serialization/deserialization for `hasRelationship`

- JSON form: `{ "kind": "hasRelationship", "relation": "...", "resourceType"?: "...", "depth"?: N, "fields"?: [...] }`
- Deserializer validates `relation` is non-empty string; `depth` is positive integer if present
- Invalid input → `PolicyParseError(ACL007)`
- Acceptance: Round-trip tests pass for all field combinations

### 33.5 Write unit, type, and integration tests (DoD 27)

- ~30 unit tests, ~8 type tests, ~17 integration tests
- Acceptance: All DoD 27 verification items pass; async path tested end-to-end
...
| 24        | Array Matchers (someMatch, contains)     | 5        | Groups 5, 8       | §16, §18, §31                 |
| 25        | Subject Enrichment Utilities             | 4        | Group 6           | §72                           |
| 26        | usePolicies / usePoliciesDeferred        | 4        | Groups 11, 25     | §73                           |
| 27        | createRoleGate Factory                   | 3        | Group 24          | §71                           |
| 28        | v3 Array Matchers (everyMatch, size)     | 5        | Group 24          | §16 v3, §18 v3, §31 v3        |
| 29        | hasResourceAttribute Policy              | 4        | Groups 3, 5, 8    | §84 policy, §18, §31          |
| 30        | withLabel Policy Wrapper                 | 4        | Groups 3, 5, 8    | §85 policy, §18, §31          |
| 31        | evaluateBatch Server-Side Batch          | 3        | Group 5           | §86                           |
| 32        | resource option on single-policy hooks   | 2        | Groups 11, 29     | §41 update                    |
| 33        | hasRelationship Policy (ReBAC)           | 5        | Groups 3,4,5,6,7,8| §04, §05, §06 (§22b), §07, §08|
| **Total** |                                          | **177**  |                   |                               |
...
Group 1 (Permission Tokens)
  ...
  |               |       |               |
  |               |       |               +---> Group 26 (usePolicies Hooks)
  |               |       |
  |               |       +---> Group 11 (React Hooks)
  |               |
  |               +---> Group 25 (Subject Enrichment Utilities)
  |                       |
  |                       +---> Group 26 (usePolicies Hooks)
...
Group 5 (Policy Evaluator)
  ...
  +---> Group 8 (Serialization)
          |
          +---> Group 24 (Array Matchers)
                  |
                  +---> Group 27 (createScopePolicy Factory)
```
...
14. **Phase 14 -- Ergonomic Improvements** (Groups 24-27): Array matchers, subject utilities, usePolicies hooks, createScopePolicy factory
