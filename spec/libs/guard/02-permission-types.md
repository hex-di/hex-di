...
A `PermissionGroup` is a convenience factory that produces an object where each key is an action name and each value is a `Permission<TResource, TAction>`. This uses a mapped type over a union of action string literals.
...
_Next: [03 - Role Types](./03-role-types.md)_
