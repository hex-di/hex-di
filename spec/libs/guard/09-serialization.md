...
### Array Matcher Serialization (someMatch, contains)

The `someMatch` and `contains` matcher kinds are fully serializable. `someMatch` contains a nested `ObjectMatcher` (a map of field names to `MatcherExpression` values); `contains` contains a single `MatcherReference`.

```typescript
const scopePolicy = hasAttribute('scopes', someMatch({
  brand:       eq(resource('brand')),
  country:     eq(resource('country')),
  indications: contains(resource('indication')),
}));

const json = serializePolicy(scopePolicy);
// {
//   "kind": "hasAttribute",
//   "attribute": "scopes",
//   "matcher": {
...
`ObjectMatcher` serializes as a plain JSON object whose keys are field names and whose values are serialized `MatcherExpression` objects. The key ordering follows the determinism guarantee (alphabetical). Deserialization validates that each value in the `matcher` object is a valid `MatcherExpression`.

The `contains` matcher serializes identically to `eq` except for the `kind` discriminant:

```typescript
const arrayMemberPolicy = hasAttribute('tags', contains(resource('requiredTag')));
const json = serializePolicy(arrayMemberPolicy);
// {
//   "kind": "hasAttribute",
//   "attribute": "tags",
//   "matcher": {
//     "kind": "contains",
//     "ref": { "kind": "resource", "path": "requiredTag" }
//   }
// }
```

Deserialization of `someMatch` validates:
- `matcher` is a non-null, non-array object
- Each value in `matcher` is a valid `MatcherExpression`
- No unknown keys are present at the `matcher` level

Deserialization of `contains` validates:
- `ref` is a valid `MatcherReference` (same rules as `eq`, `neq`, `gte`, `lt`)

### v3 Matcher Serialization (everyMatch, size)

The `everyMatch` and `size` matcher kinds are fully serializable.

`everyMatch` serializes identically to `someMatch` except for the `kind` discriminant:

```typescript
const certifiedPolicy = hasAttribute('assignedReviewers', everyMatch({ certified: eq(literal(true)) }));
const json = serializePolicy(certifiedPolicy);
// {
//   "kind": "hasAttribute",
//   "attribute": "assignedReviewers",
//   "matcher": {
//     "kind": "everyMatch",
//     "matcher": { "certified": { "kind": "eq", "ref": { "kind": "literal", "value": true } } }
//   }
// }
```

`size` contains a nested `MatcherExpression` for the length comparison:

```typescript
const atLeastTwoScopes = hasAttribute('scopes', size(gte(literal(2))));
const json = serializePolicy(atLeastTwoScopes);
// {
//   "kind": "hasAttribute",
//   "attribute": "scopes",
//   "matcher": {
...
Deserialization of `everyMatch` validates (same rules as `someMatch`):
- `matcher` is a non-null, non-array object
- Each value in `matcher` is a valid `MatcherExpression`

Deserialization of `size` validates:
- `matcher` is a valid `MatcherExpression`

### v3 Policy Serialization (hasResourceAttribute, labeled)

`hasResourceAttribute` serializes with the same shape as `hasAttribute` except for the `kind` discriminant:

```typescript
const isDraft = hasResourceAttribute('status', eq(literal('draft')));
const json = serializePolicy(isDraft);
// {
//   "kind": "hasResourceAttribute",
//   "attribute": "status",
//   "matcher": { "kind": "eq", "ref": { "kind": "literal", "value": "draft" } }
// }
```

`labeled` serializes as a three-field object preserving the inner policy:

```typescript
const namedPolicy = withLabel('ContentPublishPolicy',
  allOf(hasPermission(ContentPerms.publish), hasRole('publisher'))
);
const json = serializePolicy(namedPolicy);
// {
//   "kind": "labeled",
...
//     "kind": "allOf",
//     "policies": [
//       { "kind": "hasPermission", "permission": "content:publish" },
//       { "kind": "hasRole", "roleName": "publisher" }
//     ]
//   }
// }
```

Deserialization of `labeled` validates:
- `label` is a non-empty string
- `policy` is a valid `PolicyConstraint`
