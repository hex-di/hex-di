...
## 44b. GuardLibraryInspectorPort (Auto-Discovery)
...
export const GuardLibraryInspectorAdapter = Object.freeze(
  createAdapter({
    ...
    requires: [],
    lifetime: "singleton",
    factory: (guardInspector: GuardInspector) => createGuardLibraryInspector(guardInspector),
  })
);
```
...
## 44c. MCP Resource URIs

The guard library exposes its inspection data as MCP (Model Context Protocol) resources, following the same pattern as other HexDI libraries. Each resource has a defined URI, response schema, and content type.
...
> **Note:** The meta-audit log for both MCP resource accesses (section 44c) and A2A skill invocations (this section) MUST maintain its own tamper-evident hash chain per the meta-audit integrity requirement in section 44c above. This ensures that the access audit trail itself cannot be silently modified.
...
_Previous: [11 - React Integration](./11-react-integration.md) | Next: [13 - Testing](./13-testing.md)_
