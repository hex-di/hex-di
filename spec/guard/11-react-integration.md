...
> **Note:** `createTestSubject()` in `@hex-di/guard-testing` already returns frozen subjects (see section 47). This ensures test subjects match production behavior.
 ...
 * Never returns `undefined`. Loading state is handled by Suspense.
 *
 * Memoized by [policy, subject] -- frozen policies are referentially
