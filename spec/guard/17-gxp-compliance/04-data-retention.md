...
The recommended minimums above are general guidance. Specific regulatory frameworks impose their own retention requirements:
...
RECOMMENDED: @hex-di/guard-validation SHOULD provide a createRetentionMonitor()
             ...
             (07-guard-adapter.md) to include retention compliance as an optional
...
REQUIREMENT: Before restored audit trail data is returned to production use,
             ...
             logged in the administrative activity log (section 64b).
             Reference: 21 CFR 11.10(c) (protection of records),
             EU GMP Annex 11 Section 7.1.
```

```
RECOMMENDED: When migrating audit trail data between storage systems (e.g., database
             migration, cloud provider migration, on-premise to cloud), organizations
             SHOULD verify data migration integrity by: (1) computing the record count
             in the source and destination and confirming they match, (2) running
             verifyAuditChain() on the destination data and confirming all chains pass,
             (3) spot-checking a sample of entries (recommended: at least 1% or 100
             entries, whichever is greater) for field-level equality between source and
             destination, and (4) documenting the migration with: source system,
             destination system, migration timestamp, operator identity, record counts,
             chain verification results, and any discrepancies found. This verification
             is in addition to the OQ re-run required by section 64a for infrastructure
             migrations. Reference: ISPE GAMP Records and Data Integrity Guide (data
             lifecycle integrity).
```
...
```
RECOMMENDED: Organizations SHOULD perform quarterly backup verification in
             addition to the annual full restore test. The quarterly
             ...
             between annual full-restore tests.
             Reference: EU GMP Annex 11 §7.1, 21 CFR 11.10(c).
```
...
RECOMMENDED: The SubjectProviderPort adapter SHOULD perform pseudonymization at the
             ...
             testing with non-pseudonymized fixtures (see 07-guard-adapter.md
             section 25 for subjectId constraints, 13-testing.md section 47
...
REQUIREMENT: Organizations MUST implement a documented archival strategy for audit
             ...
             (a) Export format: Archived audit trail data MUST be exported in JSON
                 ...
                 AuditExportManifest (section 36 in 09-serialization.md). Each
             ...
             (f) Archival event logging: Each archival operation MUST be logged
                 in the administrative event log (section 64b) with: archive
...
REQUIREMENT: When `gxp: true`, the audit trail backing store MUST implement automated
             ...
             (d) The capacity status MUST be surfaced through `createGuardHealthCheck()`
                 ...
                 `GuardHealthCheckResult` (see 07-guard-adapter.md).
