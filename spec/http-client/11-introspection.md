...
interface HttpClientInspectorConfig {
   ...
   * time that an HttpAuditTrailPort adapter from @hex-di/guard is registered in the
...
REQUIREMENT: When captureBodySnapshot is "request-only" or "request-and-response",
             ...
             MUST be subject to credential redaction when @hex-di/guard's
             withCredentialProtection() combinator is active. maxPreviewBytes MUST NOT
...
| **Full GxP (guard)** | SHA-256         | Cryptographic integrity for regulatory audit trails | GxP environments requiring 21 CFR Part 11 or EU GMP Annex 11 compliance |

The basic layer (`computeHttpEntryHash`, `verifyHistoryChain`) is always available in `@hex-di/http-client`. The full GxP layer requires `@hex-di/guard`'s `HttpAuditTrailPort` (§91-§97 of this spec), which provides SHA-256 hash chains, write guarantees, and regulatory-grade persistence.
...
REQUIREMENT: FNV-1a hash chains MUST NOT be relied upon as the sole audit integrity
             ...
             For regulatory-grade cryptographic audit integrity, deploy @hex-di/guard
             which provides SHA-256 hash chains via HttpAuditTrailPort (§91-§97 of this spec).
...
REQUIREMENT: When the HTTP client inspector is configured with gxp: true in
             ...
             MUST verify at construction time that an HttpAuditTrailPort adapter from
             @hex-di/guard is registered in the container. If HttpAuditTrailPort is
             not registered, the factory MUST throw a ConfigurationError with the
             message: "GxP mode requires @hex-di/guard HttpAuditTrailPort to be
...
The HTTP client defines `HttpAuditSink` (this section) and the guard spec defines `HttpAuditTrailPort` (§91-§92 of this spec). These serve different roles:

| Aspect               | `HttpAuditSink` (http-client)                                   | `HttpAuditTrailPort` (guard)                               |
| -------------------- | --------------------------------------------------------------- | ---------------------------------------------------------- |
...
| **Crash recovery**   | Retry queue only (in-memory, lost on process crash)             | Write-ahead log (WAL) survives process crash (guard §61.5) |
| **When to use**      | Non-GxP environments, lightweight audit, development            | GxP environments, regulatory compliance                    |

> **WAL limitation:** The `HttpAuditSink` retry queue is an in-memory mechanism. It recovers from transient sink failures (e.g., database connection blips) but does **not** survive process crashes. For full GxP crash recovery, deploy `@hex-di/guard` which provides a write-ahead log (WAL, guard §61.5) that persists audit entries to disk before acknowledging writes. The WAL guarantees that no audit entries are lost even if the process terminates unexpectedly.

### Bridging Pattern

When deploying `@hex-di/guard` alongside `@hex-di/http-client`, use a bridging adapter that wraps `HttpAuditTrailPort` as an `HttpAuditSink`. This routes HTTP audit entries into the guard's SHA-256 chain:

```typescript
function createGxPAuditSinkAdapter(auditTrail: HttpAuditTrailPort): HttpAuditSink {
  return {
    write(entry: HttpHistoryEntry): Result<void, AuditSinkWriteError> {
      // Bridge: convert HttpHistoryEntry to guard audit entry format
      // and record via the guard's SHA-256 chain
...
Without `@hex-di/guard`, `HttpAuditSink` can be used standalone for lightweight audit logging:
...
> **Cross-reference:** For full GxP audit trail capabilities, see §91-§92 of this spec (HTTP Audit Trail) and §82 (Cross-Chain Integrity Verification in this document's [17 - GxP Compliance Guide](./17-gxp-compliance.md)).
