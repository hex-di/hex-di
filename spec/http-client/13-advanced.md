...
> **Extended in §112:** Formal `CorsPolicy` type definition, `withCorsHardening()` combinator, and `CorsViolationError` types are specified in §112 (GxP Compliance Audit v5.0 Remediations). This section provides the foundational CORS hardening guidance; §112 provides the concrete types and combinator API.
...
REQUIREMENT: When `gxp: true` is set and @hex-di/http-client operates in a browser
             context (platform adapter is FetchHttpClientAdapter running in a browser
...
When platform adapters support HTTP/2 (RFC 9113) or HTTP/3 (RFC 9114), additional security considerations apply to GxP deployments. These protocol versions introduce connection multiplexing, header compression, and UDP-based transport (HTTP/3) that affect audit traceability, TLS requirements, and performance qualification.
...
```
REQUIREMENT: When the platform adapter negotiates HTTP/2 or HTTP/3, the
...
REQUIREMENT: When HTTP/2 server push is enabled, pushed responses MUST be
             ...
             by the platform adapter, server push MUST be disabled for GxP
...
REQUIREMENT: HTTP/3 connections MUST satisfy the same TLS requirements as
             ...
             connections. The platform adapter MUST expose TLS metadata (version,
...
REQUIREMENT: When HTTP/2 connection coalescing is active (multiple origins
             ...
             shared. The platform adapter MUST expose per-origin certificate
             metadata.
...
RECOMMENDED: GxP deployments SHOULD include protocol version (HTTP/1.1, h2,
             ...
             SHOULD be captured from the platform adapter's connection metadata
