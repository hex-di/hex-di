...
```
@hex-di/mcp (Framework -- spec/mcp/)        @hex-di/devtools (Server -- spec/devtools/)
+-------------------------+                  +------------------------------+
...
- **`spec/devtools/`** -- The DevTools standalone server uses `@hex-di/mcp` to provide MCP capabilities alongside the visual dashboard. The 34 resources, 18 tools, and 5 prompts that expose container/library inspection data are defined in `spec/devtools/`, not here.
