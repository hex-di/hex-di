...
| Field | Value |
|-------|-------|
| Document ID | SPEC-REACT-PRC-004 |
...
This document defines the document control approach for all specification documents in the `@hex-di/result-react` specification suite. It explains how Git-based version control satisfies document control requirements for GxP compliance.

## Scope

This policy applies to all documents in the `spec/packages/result/react/` directory, including:
...
git log -1 --format="%H %ai" -- spec/packages/result/react/<file>.md

# Original author (who created the file)
git log --diff-filter=A --format="%an" -- spec/packages/result/react/<file>.md

# Full revision history
git log --oneline --follow -- spec/packages/result/react/<file>.md

# All authors who have modified this document
git log --format="%an" -- spec/packages/result/react/<file>.md | sort -u

# Approval evidence (PR merge commits)
git log --merges --first-parent main -- spec/packages/result/react/<file>.md

# Per-line attribution
git blame spec/packages/result/react/<file>.md
```
...
New document identifiers must follow the `SPEC-REACT-{CAT}-{NNN}` format and be registered in both documents.
